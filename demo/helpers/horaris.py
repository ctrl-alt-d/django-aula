...
def generaFitxerKronowin(
    path, nivellsCursosGrups, nivellsMatins, frangesMatins, frangesTardes, override
):
    # Format:
    ...
    horaris_matins = generaHoraris(
        [g for g in nivellsCursosGrups if g[0] in nivellsMatins], frangesMatins, "M"
    )
    horaris_tardes = generaHoraris(
        [g for g in nivellsCursosGrups if g[0] not in nivellsMatins], frangesTardes, "T"
    )
...
def generaHoraris(nivellsCursosGrups, franges, mati_tarda):
    ...
    for nivell, GrupsCursos in nivellsCursosGrups:
        for curs, Grups in GrupsCursos:
            for grup in Grups:
                tots_els_grups.append(
                    "{nivell}{curs}{grup}".format(nivell=nivell, grup=grup, curs=curs)
                )

    n_profes_calen = len(tots_els_grups) + 1

    professors = [
        "{0}{1}".format(mati_tarda, n_professor)
        for n_professor in range(n_profes_calen)
    ]
