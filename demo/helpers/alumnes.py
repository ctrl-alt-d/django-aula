...
def getRandomNomICognoms():
    return random.choice(nomsICognoms.noms).capitalize(), "{c1} {c2}".format(
        c1=random.choice(nomsICognoms.cognoms).capitalize(),
                ...
                for grup in Grups:
                    for _ in range(10, random.randint(20, 35)):
                        ...
                        if random.randint(1, 3) < 3 or len(llista_responsables) < 150:
                            ...
                            nif1 = llista_responsables[
                                random.randint(0, len(llista_responsables) - 1)
                            ]
                            nif2 = llista_responsables[
                                random.randint(0, len(llista_responsables) - 1)
                            ]
                        row = (
                            ##,
                            1,
                            # "00_IDENTIFICADOR DE L'ALUMNE/A",
                            "{ralc}".format(ralc=ralc),
                            # "01_NOM",
                            "{cognom}, {nom}".format(cognom=cognom1, nom=nom1),
                            # "02_DATA NAIXEMENT",
                            random_date(
                                date(year=year1, month=1, day=1),
                                date(year=year2, month=1, day=1),
                            ).strftime("%d/%m/%Y"),
                            # "03_RESPONSABLE 1",
                            "{cognom}, {nom}".format(cognom=cognom2, nom=nom2),
                            # "04_TELÈFON RESP. 1",
                            "+34 XXXXXXX",
                            # "05_MÒBIL RESP. 1",
                            "+34 XXXXXXX",
                            # "06_ADREÇA ELECTR. RESP. 1",
                            "{correu}@mailintaor.com".format(correu=slugify(nom2)),
                            # "07_RESPONSABLE 2",
                            "{cognom}, {nom}".format(cognom=cognom3, nom=nom3),
                            # "08_TELÈFON RESP. 2",
                            "+34 XXXXZZZZ",
                            # "09_MÒBIL RESP. 2",
                            "+34 XXXXLLL",
                            # "10_ADREÇA ELECTR. RESP. 2",
                            "{correu}@mailintaor.com".format(correu=slugify(nom3)),
                            # "11_ADREÇA",
                            "c/ de l'aviador {nom} {cognom}".format(
                                cognom=cognom3, nom=nom3
                            ),
                            # "12_LOCALITAT",
                            "L'Armentera",  # TODO: llista de localitats
                            # "13_MUNICIPI",
                            "Albanyà",  # TODO: llista de localitats
                            # "14_CORREU ELECTRÒNIC",
                            "{correu}@mailintaor.com".format(correu=slugify(nom1)),
                            # "15_ALTRES TELÈFONS",
                            "+34 XXXXYZZY",
                            # "16_CENTRE PROCEDÈNCIA",
                            "La Salle",
                            # "17_GRUPSCLASSE",
                            "{nivell}{curs}{grup}".format(
