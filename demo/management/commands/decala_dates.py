...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # mou_cursos
        Curs.objects.update(
            data_inici_curs=F("data_inici_curs") + timedelta(days=dies_a_moure),
            data_fi_curs=F("data_fi_curs") + timedelta(days=dies_a_moure),
        )

        # mou imparticions
        Impartir.objects.update(
            dia_impartir=F("dia_impartir") + timedelta(days=dies_a_moure)
        )
        self.stdout.write(
            "Dates decalades correctament. Data anterior:{0}, nova data{1}".format(
                data_inici_curs_is.strftime("%d/%m/%Y"),
                (data_inici_curs_is + timedelta(days=dies_a_moure)).strftime(
                    "%d/%m/%Y"
                ),
            )
        )
