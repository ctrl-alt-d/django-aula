...
from aula.apps.usuaris.models import Group, Professor
...
class TestUtils:
    ...
    def crearProfessor(self, nomUsuari, passwordUsuari, mail="mail@mailserver.com"):
        ...
        profe1 = Professor.objects.create(
            username=nomUsuari, password=passwordUsuari
        )  # type: Professor
    ...
    def generarEstatsControlAssistencia(self):
        ...
        estats["p"] = EstatControlAssistencia.objects.create(
            codi_estat="P", nom_estat="Present"
        )
        estats["f"] = EstatControlAssistencia.objects.create(
            codi_estat="F", nom_estat="Falta"
        )
        estats["r"] = EstatControlAssistencia.objects.create(
            codi_estat="R", nom_estat="Retard"
        )
        estats["j"] = EstatControlAssistencia.objects.create(
            codi_estat="J", nom_estat="Justificada"
        )
        return estats
    ...
    def omplirAlumnesHora(self, alumnes, horaAImpartir):
        ...
        for alumne in alumnes:
            ControlAssistencia.objects.create(alumne=alumne, impartir=horaAImpartir)
