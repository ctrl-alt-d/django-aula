...
def calculate_my_time_off(user):
    if user.is_anonymous:
        ...
        m = (
            settings.CUSTOM_TIMEOUT_GROUP.get(g.name, settings.CUSTOM_TIMEOUT)
...
def getSoftColor(obj):
    strc = unicode(obj) + "con mucha marcha"
    i = 0
    j = 77
    for s in strc:
        i += (103 * ord(s)) % 2001
...
def getImpersonateUser(request):
    user = (
        User.objects.get(pk=request.session["impersonacio"])
...
def initComplet():
    ...
    from aula.apps.horaris.models import Festiu, Horari
    ...
    from aula.apps.presencia.models import ControlAssistencia, Impartir, NoHaDeSerALAula
    from aula.apps.relacioFamilies.models import Responsable
    from aula.apps.sortides.models import NotificaSortida, Pagament
    from aula.apps.tutoria.models import (
        ...
        Tutor,
        TutorIndividualitzat,
    )
    from aula.apps.usuaris.models import AlumneUser, LoginUsuari, ResponsableUser
    ...
    try:
        ...
        Alumne.objects.filter(estat_sincronitzacio="MAN").update(
            estat_sincronitzacio=""
        )

        # Manté els pagaments fets, però desvinculats dels alumnes que són baixa
        Pagament.objects.filter(alumne__in=esborrar, pagament_realitzat=True).update(
            alumne=None
        )
        ...
        SeguimentTutorialRespostes.objects.filter(
            seguiment_tutorial__alumne__in=esborrar
        ).delete()
        ResumAnualAlumne.objects.filter(
            seguiment_tutorial__alumne__in=esborrar
        ).delete()
        ...
        Preinscripcio.objects.filter(
            matricula__isnull=True, any__lt=avui.date().year
        ).delete()
    ...
    Pagament.objects.filter(
        quota__any__lt=avui.date().year,
        pagament_realitzat=False,
        quota__tpv__nom="centre",
    ).delete()
    ...
    ResponsableUser.objects.filter(responsable__data_baixa__isnull=True).update(
        is_active=True
    )
    # Desactiva usuaris responsable donats de baixa
    ResponsableUser.objects.filter(responsable__data_baixa__isnull=False).update(
        is_active=False
    )
...
class processInitComplet(Thread):
    ...
    def run(self):
        ...
        if not bool(errors):
            infos.append("Inicialització finalitzada")
            missatge = FI_INITDB
        else:
            infos.append("Errors a la inicialització")
