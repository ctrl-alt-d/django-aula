...
class NoCacheMiddleware:
    ...
    def process_response(self, request, response):
        if response and type(response) is not type:
            # if hasattr(request, 'session'): request.session.set_expiry(1500)
            response["Pragma"] = "no-cache"
            response["Cache-Control"] = (
                "no-cache must-revalidate proxy-revalidate no-store"
            )
        return response
    ...
    def process_request(self, request):
        if request.user.is_authenticated:
            if "lastRequest" in request.session:
                elapsedTime = datetime.datetime.now() - datetime.datetime.strptime(
                    request.session["lastRequest"], "%c"
                )
