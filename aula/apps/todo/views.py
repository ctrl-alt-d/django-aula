...
def list(request):
    ...
    capcelera.contingut = "{0}".format("Tasques importants pendents")
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 25
    capcelera.contingut = "Tasca"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 50
    capcelera.contingut = "Contingut"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 5
    capcelera.contingut = "Esborrar"
    taula.capceleres.append(capcelera)
    ...
    for todo in ToDo.objects.filter(propietari=request.user, estat="P", prioritat="V"):
        ...
        camp.contingut = "{0}".format(todo.data)
        filera.append(camp)

        camp = tools.classebuida()
        camp.contingut = "{0}".format(todo.tasca)
        camp.enllac = todo.enllac
        filera.append(camp)

        camp = tools.classebuida()
        camp.contingut = "{0}".format(todo.informacio_adicional)
        filera.append(camp)
    ...
    capcelera = tools.classebuida()
    capcelera.amplade = 20
    capcelera.contingut = "{0}".format("Tasques Pendents")
    taula.capceleres.append(capcelera)
    ...
    for todo in ToDo.objects.filter(propietari=request.user, estat="P").exclude(
        prioritat="V"
    ):
        filera = []
    ...
    capcelera = tools.classebuida()
    capcelera.amplade = 20
    capcelera.contingut = "{0}".format("Tasques Realitzades")
    taula.capceleres.append(capcelera)
...
def edita(request, pk=None):
    ...
    if request.method == "POST":
        ...
        form.base_fields["data"].widget = forms.DateTimeInput(
            attrs={"class": "DateTimeAnyTime"}
        )
