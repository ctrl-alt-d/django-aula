...
def impartir_post_delete(sender, instance, **kwargs):
    # Esborrar la reserva
    reserva_compartida = (
        bool(instance.reserva) and instance.reserva.impartir_set.count() > 1
    )
...
def impartir_post_save(sender, instance, created, **kwargs):
    ...
    cal_assignar_nova_reserva = (
        not te_reserva and aula_informada_a_l_horari and not es_reserva_manual
    )
    if cal_assignar_nova_reserva:
        ...
        list(reserves.filter(es_reserva_manual=False))
...
def impartir_despres_de_passar_llista(instance):
    ...
    if instance.professor_passa_llista != instance.horari.professor:
        ...
        text_missatge = missatge.format(
            unicode(instance), unicode(instance.horari.professor)
        )
        ...
        msg.envia_a_usuari(
            usuari=instance.professor_passa_llista.getUser(), importancia="VI"
        )
