...
def controlAssistencia_clean(instance):
    ...
    instance.instanceDB = (
        None if not isUpdate else instance.__class__.objects.get(pk=instance.pk)
    )
    ...
    if (
        instance.instanceDB
        and instance.instanceDB.estat == instance.estat
        and instance.instanceDB.comunicat != instance.comunicat
    ):
    ...
    if user:
        instance.professor = User2Professor(user)

    #
    # Només es poden modificar assistències
    #
    nMaxDies = settings.CUSTOM_PERIODE_MODIFICACIO_ASSISTENCIA
    if isUpdate and instance.impartir.dia_impartir < (
        dt.date.today() - dt.timedelta(days=nMaxDies)
    ):
        errors.setdefault(NON_FIELD_ERRORS, []).append(
            """Aquest control d'assistència és massa antic per ser modificat (Té més de {0} dies)""".format(
                nMaxDies
            )
        )
    ...
    socTutor = (
        hasattr(instance, "professor")
        and instance.professor
        and instance.professor in tutors
    )
    daqui_2_hores = dt.datetime.now() + dt.timedelta(hours=2)
    daqui_15_dies = dt.datetime.now() + dt.timedelta(days=15)
    if (
        isUpdate
        and bool(instance.estat)
        and (not socTutor and instance.impartir.diaHora() > daqui_2_hores)
    ):
        errors.setdefault(NON_FIELD_ERRORS, []).append(
            """Encara no es pot entrar aquesta assistència 
                                    (Falta {0} per poder-ho fer )""".format(
                timesince_filter(
                    dt.datetime.now(),
                    instance.impartir.diaHora() - dt.timedelta(hours=2),
                )
            )
        )
    if (
        isUpdate
        and bool(instance.estat)
        and (socTutor and instance.impartir.diaHora() > daqui_15_dies)
    ):
        errors.setdefault(NON_FIELD_ERRORS, []).append(
            """Encara no es pot entrar aquesta assistència 
                                    (Falta {0} per poder-ho fer )""".format(
                timesince_filter(
                    dt.datetime.now(),
                    instance.impartir.diaHora() - dt.timedelta(days=15),
                )
            )
        )
    ...
    justificadaDB = (
        ...
        and instance.instanceDB.estat.codi_estat.upper() == "J"
    )
    justificadaAra = instance.estat and instance.estat.codi_estat.upper() == "J"
    posat_pel_tutor = (
        instance.instanceDB
        and instance.instanceDB.professor
        and instance.instanceDB.professor in tutors
    )
    ...
    if (
        settings.CUSTOM_NOMES_TUTOR_POT_JUSTIFICAR
        and justificadaAra
    ...
    if not justificadaDB and justificadaAra:
        ...
        dins_ambit_carta = (
            CartaAbsentisme.objects.exclude(carta_esborrada_moment__isnull=False)
            .filter(alumne=instance.alumne, data_carta__gte=data_control_mes_3)
            .exists()
        )
        if dins_ambit_carta:
            errors.setdefault(NON_FIELD_ERRORS, []).append(
    ...
    if user:
        ...
        if instance.professor:
            professors_habilitats.append(instance.professor.pk)
        if instance.impartir.horari.professor:
            professors_habilitats.append(instance.impartir.horari.professor.pk)
        if instance.impartir.professor_guardia:
            professors_habilitats.append(instance.impartir.professor_guardia.pk)
        if user.pk not in professors_habilitats:
            errors.setdefault(NON_FIELD_ERRORS, []).append(
                """Només el professor de l'assignatura, 
...
def controlAssistencia_pre_save(sender, instance, **kwargs):
    ...
    if (
        ...
        and instance.estat
        and instance.instanceDB.estat != instance.estat
        and instance.estat.codi_estat in ["F", "R", "J"]
    ):
...
def controlAssistencia_post_save(sender, instance, created, **kwargs):
    ...
    for x in sancio:
        NoHaDeSerALAula.objects.create(
            control=instance, motiu=NoHaDeSerALAula.EXPULSAT_DEL_CENTRE, sancio=x
        )
    ...
    for x in [s for s in sortida if not s.alumnes_a_l_aula_amb_professor_titular]:
        NoHaDeSerALAula.objects.create(
            control=instance, motiu=NoHaDeSerALAula.SORTIDA, sortida=x
        )
