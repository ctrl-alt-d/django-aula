...
def faltesAssistenciaEntreDatesGrupRpt(grup, dataDesDe, dataFinsA):
    q_grup = Q(impartir__horari__grup=grup)

    q_dies = Q(impartir__dia_impartir__gte=dataDesDe) & Q(
        impartir__dia_impartir__lte=dataFinsA
    )

    controls = ControlAssistencia.objects.filter(q_grup & q_dies)

    alumnes = Alumne.objects.filter(
        controlassistencia__pk__in=controls.values_list("pk", flat=True)
    ).distinct()
    ...
    taula.titol.contingut = "Resum assistència de {0} entre {1} i {2} ".format(
        grup,
    ...
    for alumne in alumnes:
        ...
        tpc = (1.0 * f) / (1.0 * ca)
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(tpc * 100)
        filera.append(camp)
    ...
    taula.titol = tools.classebuida()
    taula.titol.contingut = "Detall assistència de {0} de {1} entre {2}".format(
        grup, dataDesDe.strftime("%d/%m/%Y"), dataFinsA.strftime("%d/%m/%Y")
    )
    taula.capceleres = []
