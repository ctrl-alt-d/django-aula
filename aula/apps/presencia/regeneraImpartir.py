    ...
    def run(self):
        ...
        try:
            ...
            if not self.franja_inici:
                self.franja_inici = FranjaHoraria.objects.order_by(
                    "hora_inici", "hora_fi"
                ).first()
            ...
            self.data_inici = (
                dia_inici_curs
                if (not self.data_inici or dia_inici_curs > self.data_inici)
                else self.data_inici
            )
            ...
            for dia in totsElsDies:
                ...
                self.str_status = "Processant dia %s (%dtpc) %d dies de %d " % (
                    unicode(dia),
                    int(tpc * 100),
                    dies_fets,
                    total_dies,
                ...
                if dia == self.data_inici:
                    franja_anterior = Q(
                        horari__hora__hora_inici__gte=self.franja_inici.hora_inici
                    )
                ...
                for horari in Horari.objects.filter(
                    ...
                    if not fora_de_rang:
                        if not dia_festa:
                            impartir_modificat, created = (
                                Impartir.objects.get_or_create(
                            ...
                            if not created:
                                ...
                                aula_impartir = (
                                    impartir_modificat.reserva.aula.pk
                                    if impartir_modificat.reserva
                                    else -1
                                )
                                canvi_aula = aula_horari != aula_impartir
                                if canvi_aula:
                                    if (
                                        impartir_modificat.reserva
                                        and not impartir_modificat.reserva.es_reserva_manual
                                    ):
                                        fake_l4_credentials = (None, True)
                                        impartir_modificat.reserva.credentials = (
                                            fake_l4_credentials
                                        )
        ...
        self.str_status = "Finalitzat" + unicode(errors)

        infos.append("Regeneració finalitzada")
        infos.append("%d horaris insertats" % nHorarisInsertats)
