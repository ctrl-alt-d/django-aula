...
def alertaAssitenciaReport(data_inici, data_fi, nivell, tpc, ordenacio):
    ...
    capcelera = tools.classebuida()
    capcelera.amplade = 10
    capcelera.contingut = "% assistència"
    taula.capceleres.append(capcelera)
    ...
    q_controls = ControlAssistencia.objects.filter(alumne__in=q_alumnes).filter(
        q_filtre
    )
    ...
    if settings.CUSTOM_NO_CONTROL_ES_PRESENCIA:
        ...
        q_p = (
            q_controls.filter(estat__codi_estat__in=("P", "R", "O"))
            .order_by()
    ...
    q_j = (
        q_controls.filter(estat__codi_estat="J")
        .order_by()
        .values_list("id", "alumne__id")
        .distinct()
    )
    q_f = (
        q_controls.filter(estat__codi_estat="F")
        .order_by()
        .values_list("id", "alumne__id")
        .distinct()
    )
    ...
    for alumne in q_alumnes.select_related("grup", "grup__curs").order_by().distinct():
        ...
        alumne.tpc = (
            (float(alumne.f) / float(alumne.ca)) * 100.0 if alumne.ca > 0 else 0
        )
        alumne.tpc_assist = (
            (float(alumne.p) / float(alumne.ca)) * 100.0 if alumne.ca > 0 else 0
        )
    ...
    def order_a(a):
        return (a.cognoms, a.nom)

    def order_ca(a):
        return (a.grup.curs.nom_curs, a.grup.nom_grup, a.cognoms, a.nom)

    def order_n(a):
        return (-1 * a.tpc, -1 * a.f)

    def order_cn(a):
        return (a.grup.curs.nom_curs, a.grup.nom_grup, -1 * a.tpc)

    order = (
        order_ca
        if ordenacio == "ca"
        else order_n if ordenacio == "n" else order_cn if ordenacio == "cn" else order_a
    )
    ...
    for alumne in sorted([a for a in alumnes if a.tpc > tpc], key=order):
        ...
        # -%--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(alumne.tpc)
        filera.append(camp)
        ...
        # -assist--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(alumne.tpc_assist)
        filera.append(camp)
...
def indicadorAbsentisme(data_inici, data_fi, nivell, tpc, recerca):
    ...
    q_alumnes = Alumne.objects.filter(
        grup__curs__nivell__nom_nivell__in=settings.CUSTOM_NIVELLS.get(nivell, [])
    ).filter(data_baixa__isnull=True)
    ...
    if settings.CUSTOM_NO_CONTROL_ES_PRESENCIA:
        # té en compte tots els dies encara que no s'hagi passat llista
        q_p = q_controls.values("alumne__id").annotate(
            total=Count("id"),
            faltes=Count("id", filter=Q(estat__codi_estat__in=recerca)),
        )
    else:
        q_p = q_controls.values("alumne__id").annotate(
            total=Count("id", filter=Q(estat__isnull=False)),
            faltes=Count("id", filter=Q(estat__codi_estat__in=recerca)),
        )
    ...
    superen = (
        q_p.annotate(per=F("faltes") * 100.0 / F("total")).filter(per__gt=tpc).count()
        if quantitat > 0
        else 0
    )
...
def indicadorsReport():
    ...
    taula.titol = tools.classebuida()
    taula.titol.contingut = "Indicadors Absentisme"
    taula.capceleres = []

    capcelera = tools.classebuida()
    capcelera.amplade = 10
    capcelera.contingut = "Curs"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 10
    capcelera.contingut = "Nivell"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 8
    capcelera.contingut = "% ind."
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 8
    capcelera.contingut = "1rTrim"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 8
    capcelera.contingut = "2nTrim"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 8
    capcelera.contingut = "3rTrim"
    taula.capceleres.append(capcelera)
    ...
    for ind in settings.CUSTOM_INDICADORS:
        ...
        # -% ind--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(tpc)
        filera.append(camp)

        # -%1rTrim--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(trim1)
        filera.append(camp)

        # -%2nTrim--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(trim2)
        filera.append(camp)

        # -%3rTrim--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(trim3)
        filera.append(camp)

        # -%Curs--------------------------------------------
        camp = tools.classebuida()
        camp.contingut = "{0:.2f}%".format(curs)
        filera.append(camp)
        ...
        writer.writerow(
            [
                ...
                quan1,
                trim1,
                (dtrim1 + undia).strftime("%d/%m/%Y"),
                ind[2],
                sup2,
                quan2,
                trim2,
                (dtrim2 + undia).strftime("%d/%m/%Y"),
                ind[3],
                sup3,
                quan3,
                trim3,
