...
from aula.apps.baixes.forms import (
    complementFormulariTriaForm,
...
def feina(request, pk_imparticio):
    ...
    head = "Feina per a {0}".format(imparticio)

    frmFact = modelform_factory(
        Feina,
        fields=(
            "feina_a_fer",
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                r"/presencia/passaLlista/{0}".format(imparticio.pk)
            )
    else:
...
def complementFormulariOmple(request, pk_professor, dia, mes, year):
    ...
    head = "Feina professor absent {0} {1}".format(professor, data)

    imparticions = Impartir.objects.filter(
        dia_impartir=data, horari__professor__pk=pk_professor
    )
    ...
    if request.method == "POST":
        formset = formsetFac(
            request.POST, queryset=Feina.objects.filter(impartir__in=imparticions)
        )
        if formset.is_valid():
            formset.save()
            for form in formset:
                if form.instance.professor_fa_guardia:
                    form.instance.impartir.professor_guardia = (
                        form.instance.professor_fa_guardia
                    )
    ...
    for form in formset:
        ...
        form.infoForm = (
            ...
            ("Assignatura", instance.impartir.horari.assignatura),
            ("Grup", instance.impartir.horari.grup),
            ("Aula", instance.impartir.horari.nom_aula),
            (
