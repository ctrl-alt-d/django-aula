...
def esFestiu(curs, dia, hora):
    ...
    q_ini = Q(data_inici_festiu__lt=dia) | (
        Q(data_inici_festiu=dia)
        & Q(franja_horaria_inici__hora_inici__lte=hora.hora_inici)
    )
    q_fin = Q(data_fi_festiu__gt=dia) | (
        Q(data_fi_festiu=dia) & Q(franja_horaria_fi__hora_inici__gte=hora.hora_inici)
    )

    if Festiu.objects.filter(q_ini & q_fin & q_festiu_curs_o_tots).count() > 0:
        return True
