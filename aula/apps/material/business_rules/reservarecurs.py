...
def reservarecurs_clean(instance):
    ...
    if instance.es_reserva_manual:
        if not instance.recurs.reservable:
            errors.setdefault("hora", []).append(
                """Material exempt de reserves. No pot ser reservat."""
            )

    # -- No es pot reservar un recurs ocupat
    if instance.es_reserva_manual:
        recursReservat = (
            ReservaRecurs.objects.filter(
                hora=instance.hora,
        ...
        if recursReservat:
            professorsQueReserven = [
                reserva.usuari.first_name + " " + reserva.usuari.last_name
                for reserva in ReservaRecurs.objects.filter(
                    hora=instance.hora,
                    recurs=instance.recurs,
                    dia_reserva=instance.dia_reserva,
                )
            ...
            errors.setdefault("hora", []).append(
                """Material ocupat en aquesta hora per """
    ...
    if instance.es_reserva_manual:
        ...
        if bool(disponibilitatHoraria) and instance.hora not in disponibilitatHoraria:
            errors.setdefault("hora", []).append(
                "No està previst que es pugui reservar aquest material en aquest horari"
            )
