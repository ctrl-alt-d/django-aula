...
class disponibilitatRecursPerFranjaForm(forms.Form):
    ...
    def clean(self):
        ...
        es_reservador = User.objects.filter(
            pk=self.user.pk, groups__name__in=["reservador"]
        ).exists()
        if not es_reservador and (
            data > darrer_dia_reserva or data < datetime.today().date()
        ):
            raise forms.ValidationError(
                "Només pots reservar a partir d'avui i fins al dia {0}".format(
                    darrer_dia_reserva
                )
            )
        ...
        franges_del_dia = FranjaHoraria.objects.filter(
            horari__impartir__dia_impartir=data
        ).order_by("hora_inici")
        # Es comprova que la franja correspongui al dia
        if not franges_del_dia or franja not in franges_del_dia:
            raise forms.ValidationError("En aquesta franja i dia no hi ha docència")

        return cleaned_data


class RecursosForm(forms.Form):
    recurs = forms.ModelChoiceField(queryset=None, empty_label=None)
...
class reservaRecursForm(ModelForm):
    ...
    def clean_hora(self):
        ...
        if (
            franja.hora_inici < primera_franja.hora_inici
            or franja.hora_fi > darrera_franja.hora_fi
        ):
            raise forms.ValidationError("En aquesta franja i dia no hi ha docència")
    ...
    class Meta:
        model = ReservaRecurs
        labels = {"motiu": "Propòsit"}
        fields = ["recurs", "dia_reserva", "hora", "motiu"]
        widgets = {"motiu": forms.Textarea, "dia_reserva": DateTextImput()}
...
class disponibilitatMassivaRecursPerFranjaForm(forms.Form):
    ...
    data_inici = forms.DateField(
        help_text="Data d'inici a consultar",
        initial=datetime.today(),
        required=True,
        widget=DateTextImput(),
    )
    ...
    data_fi = forms.DateField(
        help_text="Data de fi a consultar",
        initial=datetime.today(),
        required=True,
        widget=DateTextImput(),
    )
    ...
    def clean(self):
        ...
        if dia_setmana_data_inici != dia_setmana_data_fi:
            raise forms.ValidationError(
                "Les dates d'inici i fi no coincideixen en el dia de la setmana (dilluns, dimarts,...)"
            )
        es_reservador = User.objects.filter(
            pk=self.user.pk, groups__name__in=["reservador"]
        ).exists()
        if not es_reservador:
            raise forms.ValidationError(
                "Aquesta acció només la pot realitzar un usuari amb permissos"
            )

        franja = self.cleaned_data["franja"]
        franges_del_dia = FranjaHoraria.objects.filter(
            horari__impartir__dia_impartir=data_inici
        ).order_by("hora_inici")
        primera_franja = franges_del_dia.first()
        darrera_franja = franges_del_dia.last()
        if (
            dia_setmana_data_inici in [5, 6]
            or franja.hora_inici < primera_franja.hora_inici
            or franja.hora_fi > darrera_franja.hora_fi
        ):
            raise forms.ValidationError("En aquesta franja i dia no hi ha docència")
...
class reservaMassivaRecursForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {"motiu": forms.Textarea, "dia_reserva": forms.HiddenInput}
