...
from aula.apps.material.business_rules.recurs import (
    recurs_clean,
)
from aula.apps.material.business_rules.reservarecurs import (
    reservarecurs_clean,
)
...
class Recurs(models.Model):
    ...
    def __str__(self):
        descripcio = (
            " - {0}".format(self.descripcio_recurs)
            if bool(self.descripcio_recurs)
            else ""
        )
        nom_i_descripcio = "{0}{1}".format(self.nom_recurs, descripcio)
...
class ReservaRecurs(models.Model):
    recurs = models.ForeignKey("material.Recurs", on_delete=models.PROTECT)
    ...
    es_reserva_manual = models.BooleanField(
        editable=False, default=False, help_text="la reserva s'ha fet manualment"
    )
    ...
    def __str__(self):
        return "{recurs} {hora_inici}-{hora_fi} {usuari}".format(
            recurs=self.recurs,
            hora_inici=self.hora_inici,
            hora_fi=self.hora_fi,
            usuari=self.usuari,
        )
...
from aula.apps.material.business_rules.recurs import (  # noqa: E402
    recurs_post_save,
    recurs_pre_delete,
    recurs_pre_save,
)
from aula.apps.material.business_rules.reservarecurs import (  # noqa: E402
    reservarecurs_post_delete,
    reservarecurs_post_save,
    reservarecurs_pre_delete,
    reservarecurs_pre_save,
)
