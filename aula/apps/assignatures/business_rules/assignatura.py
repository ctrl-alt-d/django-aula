...
def assignatura_clean(instance):
    ...
    if instance.tipus_assignatura is None:
        ...
        if instance.curs and instance.curs.nivell.pk in [
            nivell.pk for nivell in cicles
        ]:
        ...
        # si és AO, Optativa, ...
        if (
            instance.codi_assignatura.startswith("OP")
            or instance.codi_assignatura.startswith("OE")
            or instance.codi_assignatura.startswith("MO")
            or instance.codi_assignatura.startswith("AO")
        ):

            try:
                op = TipusDAssignatura.objects.get(
                    tipus_assignatura__startswith="Opcional"
                )
        ...
        if instance.curs is not None and instance.curs.nom_curs == "ACO/UEC":
            try:
                ao = TipusDAssignatura.objects.get(
                    tipus_assignatura__startswith="Aula Oberta"
                )
