...
class PresenciaSetmanalTestCase(TestCase):
    ...
    def generaBD(self):
        ...
        self.primerESOA = Grup.objects.create(
            nom_grup="A", curs=primerESO
        )  # type: Grup
        alumne = Alumne.objects.create(
            ralc=100, grup=self.primerESOA, nom="Xevi", cognoms="Petit"
        )  # type: Alumne
        self.alumne = alumne
        alumne2 = Alumne.objects.create(
            ralc=100, grup=self.primerESOA, nom="Joan", cognoms="Serra"
        )  # type: Alumne
        # Crear un profe.
        ...
        profe1 = Professor.objects.create(
            username="SrCastanya", password="patata"
        )  # type: Professor
        ...
        profe2 = Professor.objects.create(
            username="SrIntrus", password="patata"
        )  # type: Professor
        ...
        profe3 = Professor.objects.create(
            username="SrTutorESOA", password="patata"
        )  # type: Professor
        ...
        tmpDS = DiaDeLaSetmana.objects.create(
            n_dia_uk=diaAnterior.isoweekday(),
            n_dia_ca=diaAnterior.weekday(),
            dia_2_lletres="XX",
            dia_de_la_setmana="dia" + str(diaAnterior.weekday()),
            es_festiu=False,
        )
        tmpFH = FranjaHoraria.objects.create(hora_inici="9:00", hora_fi="10:00")

        matresPrimerESO = Assignatura.objects.create(
            nom_assignatura="Mates", curs=primerESO
        )
        ...
        impartirDilluns = Impartir.objects.create(
            horari=horari, professor_passa_llista=profe1, dia_impartir=dillunsActual
        )
        self.impartirDilluns = impartirDilluns

        estatPresent = EstatControlAssistencia.objects.create(
            codi_estat="P", nom_estat="Present"
        )
        ...
        ControlAssistencia.objects.create(
            alumne=alumne, estat=estatPresent, impartir=impartirDilluns
        )

        ControlAssistencia.objects.create(alumne=alumne2, impartir=impartirDilluns)

    def _canviEstat(self, client, estatInicial):
        # type: () => HttpResponse
        url = (
            self.urlBase
            + "presenciaSetmanal/modificaEstatControlAssistencia/{2}/{0}/{1}".format(
                self.alumne.pk,
                self.impartirDilluns.pk,
                urllib.parse.quote(estatInicial),
            )
        )
        # print ("debug:::", url)
...
class PresenciaSetmanalTestCaseAillament1(PresenciaSetmanalTestCase):
    ...
    def test_client(self):
        ...
        c = Client()
        c.post(
            "http://localhost:8000/usuaris/login/",
            {"usuari": "SrCastanya", "paraulaDePas": "patata"},
        )
        response2 = c.get(
            "http://127.0.0.1:8000/presenciaSetmanal/" + str(self.primerESOA.pk) + "/"
        )
        f = open("prova.html", "w")
    ...
    def test_comprovaNElementsTaula(self):
        ...
        self.assertEquals(
            nAlumnes * nAssignatures,
            nceles,
            "El número d'elements de la taula no coincidèixen.",
        )
    ...
    def test_client_nopuc_modificar_assistencia_altri(self):
        ...
        c = Client()
        c.post(
            self.urlBase + "usuaris/login/",
            {"usuari": "SrIntrus", "paraulaDePas": "patata"},
        )
    ...
    def test_client_nopuc_posar_tot_a_present(self):
        ...
        c.get(
            self.urlBase
            + "presenciaSetmanal/modificaEstatControlAssistenciaGrup/P/"
            + str(self.impartirDilluns.pk)
        )
        ...
        self.assertGreater(
            len(controls.exclude(estat__codi_estat="P")),
            0,
            "Hauria d'haver-hi estats no presents",
        )
