...
def detallgrup(request, grup_id, dataReferenciaStr="", nomesPropies=None):
    ...
    if nomesPropies is not None and nomesPropies == "True":
        ...
        assistencies = ControlAssistencia.objects.filter(
            ...
            "alumne__cognoms",
            "impartir__horari__dia_de_la_setmana__n_dia_ca",
    ...
    hores = Impartir.objects.filter(pk__in=assistencies.values("impartir")).order_by(
    ...
    for assistencia in assistencies:
        ...
        if assistencia.estat is not None:
            idEstat = assistencia.estat.id
            # print("Estat", idEstat, "idAlumne", idAlumne, "idHora", idHora)

        if mah.get(str(idAlumne), None) is None:
            # print "\nidAlumne que no esta al grup:" + str(idAlumne) + "idHora" + str(idHora) + \
            #            "estat:" + str(assistencia.estat_id)
            mah[str(idAlumne)] = {}
            dAlumnes[str(idAlumne)] = AlumneMemoria(
                idAlumne, assistencia.alumne.nom, assistencia.alumne.cognoms
            )

        mah[str(idAlumne)][str(idHora)] = AssistenciaAlumne(
            idEstat, idAlumne, idHora, estats
        )
...
def modificaEstatControlAssistencia(request, codiEstat, idAlumne, idImpartir):
    ...
    try:
        ...
        Accio.objects.create(
            ...
            text="""Passar llista, presenciaSetmanal: {0}.""".format(impartir),
        )
...
def modificaEstatControlAssistenciaGrup(request, codiEstat, idImpartir):
    ...
    try:
        ...
        for assistencia in assistencies:
            ...
            _modificaEstatControlAssistencia(
                nouEstat, assistencia, profeActual, user, credentials
            )
...
def _modificaEstatControlAssistencia(
    nouEstat, controlAssistencia, profeActual, user, credentials
):
...
def _comprovarQueLaHoraPertanyAlProfessorOError(credentials, impartir):
    ...
    if not (l4 or pertany_al_professor):
        raise ProfeNoPot(
            {
                NON_FIELD_ERRORS: [
                    "Error al modificar l'assistència del grup, el profe no és propietari de l'hora assignada",
                ],
            }
        )
