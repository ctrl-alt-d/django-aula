...
class EscollirGrupForm(forms.Form):
    grup_list = forms.ModelChoiceField(
        label="Grup d'alumnes",
        queryset=None,
        required=True,
    )
    nomesPropies = forms.BooleanField(label="Només hores pròpies", required=False)

    def __init__(self, professor, *args, **kwargs):
        super(EscollirGrupForm, self).__init__(*args, **kwargs)
        self.fields["grup_list"].queryset = Grup.objects.filter(
