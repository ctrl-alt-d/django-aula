...
class Matricula(models.Model):
    ...
    curs = models.ForeignKey(
        Curs, verbose_name="Curs on matricular-se", on_delete=models.PROTECT
    )
    ...
    preinscripcio = models.ForeignKey(
        Preinscripcio, on_delete=models.PROTECT, null=True, blank=True
    )
    acceptar_condicions = models.BooleanField("Accepto condicions de matrícula")
    acceptacio_en = models.DateTimeField(null=True)
    confirma_matricula = models.CharField(
        "Confirmo matrícula", max_length=1, choices=CONF_CHOICES, null=True, blank=True
    )
    ...
    bonificacio = models.CharField(
        "Tipus de bonificació taxes", max_length=1, choices=BONIF_CHOICES, default="0"
    )
    ...
    centre_de_procedencia = models.CharField(
        "Centre de procedència", max_length=50, null=True, blank=True
    )
    data_naixement = models.DateField("Data de naixement", default=None)
    alumne_correu = models.EmailField(
        "Correu de l'alumne", help_text="Correu de notificacions de l'alumne", null=True
    )
    ...
    rp1_nom = models.CharField(
        "Nom complet 1r responsable", max_length=250, default="", blank=True
    )  # responsable 1
    rp1_telefon = models.CharField(
        "Telèfon 1r responsable", max_length=15, default="", blank=True
    )
    rp1_correu = models.EmailField("Correu 1r responsable", default="", blank=True)
    rp2_nom = models.CharField(
        "Nom complet 2n responsable", max_length=250, null=True, blank=True
    )  # responsable 2
    rp2_telefon = models.CharField(
        "Telèfon 2n responsable", max_length=15, null=True, blank=True
    )
    ...
    def getPagament(self):
        p = QuotaPagament.objects.filter(
            alumne=self.alumne,
            quota__any=self.any,
...
class Document(models.Model):
    ...
    matricula = models.ForeignKey(
        Matricula, on_delete=models.PROTECT, blank=True, null=True
    )
    ...
    def __str__(self):
        return str(self.fitxer.name) + (
            (" " + str(self.matricula)) if self.matricula else ""
        )
