...
from aula.apps.sortides.models import TPV, Quota, QuotaCentre, QuotaPagament
...
def comunicaMatricula(tipus, curs, alumne, connection=None):
    ...
    if (
        alumne.edat() >= 18
        or not alumne.responsables.exists()
        or not alumne.get_correus_relacio_familia(checkMajorEdat=False)
    ):
    ...
    if settings.DEBUG:
        print(
            "Enviant comunicació sobre la matrícula a {0}".format(
                (responsable.nom + " " + responsable.cognoms)
                if responsable
                else (alumne.nom + " " + alumne.cognoms)
            )
        )
    try:
        r = enviamail(
...
def quotaSegüentCurs(nomtipus, nany, alumne):
    ...
    if not quotacurs:
        ...
        if (
            ...
            or quotacurs[0].curs is not None
        ):
...
def activaResponsables(al, preinscripcio=None):
    ...
    if preinscripcio:
        ...
        if resp1 and resp1.dni:
            dades1 = {"dni": resp1.dni}
        if resp2 and resp2.dni:
            dades2 = {"dni": resp2.dni}
...
def alumne2Mat(alumne, nany=None):
    ...
    if mat:
        ...
        if p:
            ...
            mat.centre_de_procedencia = (
                p.centreprocedencia[0:50] if p.centreprocedencia else ""
            )
            ...
            mat.centre_de_procedencia = (
                alumne.centre_de_procedencia[0:50]
                if alumne.centre_de_procedencia
                else ""
            )
...
def updateAlumne(alumne, mat):
    ...
    dades_resp2 = {}
    for k in dades_mat:
        if k.startswith("rp1_"):
            dades_resp1[k[4:]] = dades_mat[k] or ""
        if k.startswith("rp2_"):
            dades_resp2[k[4:]] = dades_mat[k] or ""
...
def getCanvis(idMat):
    ...
    for k in dades_mat:
        ...
        if (
            not bool(resp1)
            and bool(dades_mat[k])
            or k.startswith("rp1_")
            and k[4:] in dades_resp1
            and diferents(dades_resp1[k[4:]], dades_mat[k])
        ):
            llista.append(k)
        if (
            not bool(resp2)
            and bool(dades_mat[k])
            or k.startswith("rp2_")
            and k[4:] in dades_resp2
            and diferents(dades_resp2[k[4:]], dades_mat[k])
        ):
            llista.append(k)
...
def enviaIniciMat(nivell, tipus, nany, ultimCursNoEmail=False, senseEmails=False):
    ...
    if tipus == "P":
        for p in Preinscripcio.objects.filter(
            ...
            curs = p.getCurs()
            if not senseEmails:
                comunicaMatricula(tipus, curs, alumne, connection)
            p.estat = "Enviada"
            p.save()
    if tipus == "A" or tipus == "C":
        for a in Alumne.objects.filter(
            ...
            if següentCurs(a) or not ultimCursNoEmail:
                if not senseEmails:
                    comunicaMatricula(tipus, a.grup.curs, a, connection)
    # tanca la connexió
    if connection:
        connection.close()
