...
def ConfContestada(alumne, nany):
    ...
    return (
        mt
        and bool(mt[0].confirma_matricula)
        and (mt[0].acceptar_condicions or mt[0].confirma_matricula == "N")
    )
...
def ConfirmacioActivada(alumne):
    ...
    return alumne.grup.curs.confirmacio_oberta and (
        not alumne.grup.curs.limit_confirmacio
        or django.utils.timezone.now().date() <= alumne.grup.curs.limit_confirmacio
    )
...
def FlagMatriculaOberta(alumne, preinscripcio=None):
    ...
    """
    if alumne.grup.curs.nivell.matricula_oberta:
        return True
    if not bool(preinscripcio):
        return False
...
def MatriculaOberta(alumne, preinscripcio=None):
    ...
    if FlagMatriculaOberta(alumne, preinscripcio):
        if not alumne.grup.curs.nivell.limit_matricula or (
            ...
            <= alumne.grup.curs.nivell.limit_matricula
        ):
            return True
        if not bool(preinscripcio):
            return False
        curs = preinscripcio.getCurs()
        return (
            not curs.nivell.limit_matricula
            or django.utils.timezone.now().date() <= curs.nivell.limit_matricula
        )
    return False
    ...
    try:
        if usuari.alumne:
            ...
            if info == "C" and not ConfContestada(usuari.alumne, nany):
                return reverse_lazy(
                    "matricula:relacio_families__matricula__confirma",
                    kwargs={"nany": nany},
                )
        return None
    except Exception:
...
def matriculaDoble(alumne, nany):
    ...
    if p:
        ...
        p = None
    if not ConfirmacioActivada(alumne) and not MatriculaOberta(alumne, p):
        return False
    mt = Matricula.objects.filter(alumne=alumne, any=nany)
    if not mt or not p:
        return False
    ...
    if not mt.acceptar_condicions or mt.confirma_matricula == "N":
        ...
        mt.estat = "A"
        mt.curs = curs
        q = QuotaCentre.objects.filter(
            curs=mt.curs, any=nany, tipus__nom=settings.CUSTOM_TIPUS_QUOTA_MATRICULA
        )
...
def situacioMat(alumne, nany):
    ...
    situacions = {
        "nomatricula": "No és període de matrícula.",
        "matpendent": "Matrícula en espera de revisió.",
        "matfinal": "Matrícula finalitzada. Procés completat. Gestiona els pagaments des de l'apartat "
        + url,
    ...
    if matriculaDoble(alumne, nany):
        return "D"
    mt = Matricula.objects.filter(alumne=alumne, any=nany)
    p = Preinscripcio.objects.filter(
        ralc=alumne.ralc, any=nany
    )  # Qualsevol preinscripció no permet confirmació
    if alumne.grup.curs.confirmacio_oberta and not p:
        if ConfContestada(alumne, nany):
            ...
            if (mt and not mt[0].preinscripcio or not mt) and (
                not alumne.grup.curs.limit_confirmacio
                or django.utils.timezone.now().date()
                <= alumne.grup.curs.limit_confirmacio
            ):
                return "C"
    p = Preinscripcio.objects.filter(
        ralc=alumne.ralc, any=nany, estat="Enviada"
    )  # Només preinscripcions actuals, última tanda
    if p:
        p = p[0]
        curs = p.getCurs()
    else:
    ...
    if FlagMatriculaOberta(alumne, p):
        ...
        if MatriculaOberta(alumne, p):
            if (not curs.nivell.preexclusiva or p) and not Preinscripcio.objects.filter(
                ralc=alumne.ralc, any=nany
            ).exclude(estat="Enviada"):
                return "M"
...
def enviamail(subject, message, from_email, to_email, connection=None):
    ...
    try:
        if to_email:
            email = EmailMessage(
                subject, message, from_email, bcc=to_email, connection=connection
            )
...
def mailMatricula(tipus, curs, email, alumne, connection=None):
    ...
    assumpte = {
        "P": "Instruccions de matrícula - {0}".format(settings.NOM_CENTRE),
        "A": "Instruccions de matrícula - {0}".format(settings.NOM_CENTRE),
        "C": "Confirmació de matrícula - {0}".format(settings.NOM_CENTRE),
        "F": "Matrícula completada - {0}".format(settings.NOM_CENTRE),
    }

    if curs.nivell.limit_matricula:
        data_mat = "La data límit és {0}.".format(
            curs.nivell.limit_matricula.strftime("%A %-d %B")
        )
    else:
        data_mat = ""
    if curs.limit_confirmacio:
        data_conf = "La data límit és {0}.".format(
            curs.limit_confirmacio.strftime("%A %-d %B")
        )
    else:
        data_conf = ""
    ...
    cosmissatge = {
        "P": "\n".join(
            [
                "El procés de matrícula ja s'ha iniciat, per realitzar la matrícula de l'alumne/a {0}, s'ha d'accedir a l'aplicació ".format(
                    alumne.nom + " " + alumne.cognoms
                )
                + "des de {0} amb l'usuari {1}.".format(
                    settings.URL_DJANGO_AULA, username
                ),
                data_mat,
            ]
        ),
        ...
        "C": "\n".join(
            [
                ...
                "Per realitzar la confirmació de matrícula de l'alumne/a {0}, s'ha d'accedir a l'aplicació ".format(
                    alumne.nom + " " + alumne.cognoms
                )
                + "des de {0} amb l'usuari {1}.".format(
                    settings.URL_DJANGO_AULA, username
        ...
        "F": "\n".join(
            [
                "La matrícula de l'alumne {0} ha finalitzat correctament.".format(
                    alumne.nom + " " + alumne.cognoms
                ),
                "Sempre podreu accedir a l'aplicació {0} amb el vostre usuari {1}".format(
                    settings.URL_DJANGO_AULA, username
                ),
    ...
    if tipus == "P":
        if curs.nivell.nom_nivell == "ESO":
            cos = "\n".join(
                [
                    cos,
                    "Durant el procés de matrícula serà necessari pujar la documentació requerida:",
    ...
    if tipus != "F":
        cos = "\n".join(
            [
                cos,
                "",
                "Si encara no ha obtingut la contrasenya, entreu a {0} per escollir-la.".format(
                    url
                ),
            ]
        )
    missatge = [
        ...
        settings.NOM_CENTRE,
        "",
        "Aquest missatge ha sigut enviat per un sistema automàtic. No respongui a aquest e-mail, el missatge no serà llegit per ningú.",
    ]
    fromuser = settings.DEFAULT_FROM_EMAIL
    if settings.DEBUG:
        print(
            "Enviant comunicació sobre la matrícula a {0}".format(
                alumne.nom + " " + alumne.cognoms
            )
        )
    try:
...
def quotaSegüentCurs(nomtipus, nany, alumne):
    ...
    try:
        ncurs = str(int(c.nom_curs) + 1)
        quotacurs = QuotaCentre.objects.filter(
            curs__nivell=c.nivell, curs__nom_curs=ncurs, any=nany, tipus__nom=nomtipus
        )
    except:
        quotacurs = None
    if not quotacurs:
        ...
        if (
            not settings.CUSTOM_QUOTA_UNICA_MATRICULA
            or quotacurs.count() != 1
            or quotacurs[0].curs != None
        ):
...
def creaAlumne(preinscripcio):
    ...
    al.correu_relacio_familia_pare = (
        preinscripcio.correu
        if al.primer_responsable == 0
        else al.correu_relacio_familia_pare
    )
    al.correu_relacio_familia_mare = (
        preinscripcio.correu
        if al.primer_responsable == 1
        else al.correu_relacio_familia_mare
    )
...
def gestionaPag(matricula, importTaxes):
    ...
    if taxes and importTaxes > 0:
        quotatax = QuotaCentre.objects.filter(
            importQuota=importTaxes, any=matricula.any, tipus=taxes
        )
        if quotatax:
            quotatax = quotatax[0]
        else:
            ...
            quotatax = Quota(
                importQuota=importTaxes,
                dataLimit=matricula.curs.nivell.limit_matricula
    ...
    if matricula.confirma_matricula == "C" and matricula.estat == "A":
        ...
        quotamat = QuotaCentre.objects.filter(
            curs=matricula.curs,
            any=matricula.any,
            tipus__nom=settings.CUSTOM_TIPUS_QUOTA_MATRICULA,
        )
        if quotamat:
            quotamat = quotamat[0]
        else:
    ...
    matricula.save()
    pag = QuotaPagament.objects.filter(
        alumne=matricula.alumne,
        quota__any=matricula.any,
    ...
    if pag:
        ...
        if pag[0].quota != quotamat and not pag.filter(pagament_realitzat=True):
            ...
            enviaMissatge(
                "Canvi de quota. Matrícula:{0} {1} {2}. Quota anterior {3}. Quota actual {4}.".format(
                    str(matricula.alumne),
                    str(matricula.curs),
                    matricula.any,
                    str(pag[0].quota),
                    str(quotamat),
            ...
            if pag[0].quota != quotamat and pag.filter(pagament_realitzat=True):
                ...
                if (
                    pag[0].quota
                    and quotamat
                    and pag[0].quota.importQuota != quotamat.importQuota
                ):
                    # incompatible
                    enviaMissatge(
                        "Quota pagada no correspon. Matrícula:{0}-{1}".format(
                            matricula.idAlumne, matricula.any
                        )
                    )
                else:
    ...
    pag = QuotaPagament.objects.filter(
        alumne=matricula.alumne, quota__any=matricula.any, quota__tipus=taxes
    )
    if pag:
        if (pag[0].quota != quotatax and not pag.filter(pagament_realitzat=True)) or (
            pag[0].quota == quotatax
            and not pag.filter(pagament_realitzat=True)
            and pag[0].fracciona != fracciona
        ):
            ...
            if pag[0].quota != quotatax and pag.filter(pagament_realitzat=True):
                ...
                enviaMissatge(
                    "Taxes pagades no corresponen. Matrícula:{0}-{1}".format(
                        matricula.idAlumne, matricula.any
                    )
                )
    else:
...
def alumne2Mat(alumne, nany=None, p=None):
    ...
    if mat:
        ...
        if p:
            ...
            mat.cp = p.cp
            mat.rp1_nom = (
                p.nomtut1 + " " + p.cognomstut1 if p.nomtut1 and p.cognomstut1 else ""
            )
            mat.rp1_telefon = p.telefon
            mat.rp1_correu = p.correu
            mat.rp2_nom = (
                p.nomtut2 + " " + p.cognomstut2 if p.nomtut2 and p.cognomstut2 else ""
            )
            mat.preinscripcio = p
            curs = p.getCurs()
            mat.curs = curs
            ...
            mat.rp1_telefon = (
                alumne.rp1_mobil if alumne.rp1_mobil else alumne.rp1_telefon
            )
            mat.rp1_correu = (
                alumne.correu_relacio_familia_pare
                if alumne.primer_responsable == 0
                else alumne.correu_relacio_familia_mare
            )
            mat.rp2_nom = alumne.rp2_nom
            mat.rp2_telefon = (
                alumne.rp2_mobil if alumne.rp2_mobil else alumne.rp2_telefon
            )
            mat.rp2_correu = (
                alumne.correu_relacio_familia_mare
                if alumne.primer_responsable == 0
                else alumne.correu_relacio_familia_pare
            )
...
def diferents(dada1, dada2):
    ...
    if (not dada1 and not dada2) or dada1 == dada2:
        return False
    return True
...
def getCanvis(idMat):
    ...
    llista = []
    if diferents(alumne.correu, mat.alumne_correu):
        llista.append("alumne_correu")
    if diferents(alumne.adreca, mat.adreca):
        llista.append("adreca")
    if diferents(alumne.localitat, mat.localitat):
        llista.append("localitat")
    if diferents(alumne.cp, mat.cp):
        llista.append("cp")
    if diferents(alumne.rp1_nom, mat.rp1_nom):
        llista.append("rp1_nom")
    if diferents(alumne.rp1_telefon, mat.rp1_telefon):
        llista.append("rp1_telefon")
    if diferents(alumne.rp1_correu, mat.rp1_correu):
        llista.append("rp1_correu")
    if diferents(alumne.rp2_nom, mat.rp2_nom):
        llista.append("rp2_nom")
    if diferents(alumne.rp2_telefon, mat.rp2_telefon):
        llista.append("rp2_telefon")
    if diferents(alumne.rp2_correu, mat.rp2_correu):
        llista.append("rp2_correu")
    return llista
...
def mat_selecciona(idcurs, nany, tipus):
    ...
    if tipus != "N" and tipus != "F":
        escollides = Q(escollides & Q(acceptar_condicions=True))
    # Confirmades
    if tipus == "C":
        escollides = Q(escollides & Q(confirma_matricula=tipus) & Q(estat="A"))
    # No Confirmades
    if tipus == "N":
        escollides = Q(escollides & Q(confirma_matricula=tipus))
    # Segons estat
    if tipus == "A" or tipus == "F":
        escollides = Q(escollides & Q(estat=tipus))
    # Segons el curs
    if idcurs:
        escollides = Q(escollides & Q(curs__id=idcurs))
    return escollides
...
def next_mat(pk, idcurs, nany, tipus):
    ...
    matricules = Matricula.objects.filter(
        Q(mat_selecciona(idcurs, nany, tipus) | Q(pk=pk))
    ).order_by("curs__nom_curs_complert", "cognoms", "nom")
    ...
    if next_id == pk:
        matricules = Matricula.objects.filter(
            mat_selecciona(idcurs, nany, tipus)
        ).order_by("curs__nom_curs_complert", "cognoms", "nom")
        if not matricules:
            return None
    return next_id
...
def inforgpd():
    ...
    """
    if not hasattr(settings, "INFORGPD") or not bool(settings.INFORGPD):
        return ""
    try:
...
def enviaIniciMat(nivell, tipus, nany, ultimCursNoEmail=False, senseEmails=False):
    ...
    connection.open()
    if tipus == "P":
        for p in Preinscripcio.objects.filter(
            codiestudis=nivell.nom_nivell,
            any=nany,
            estat="Assignada",
            ...
            curs = p.getCurs()
            if not senseEmails:
                mailMatricula(tipus, curs, p.correu, alumne, connection)
            p.estat = "Enviada"
            p.save()
    if tipus == "A" or tipus == "C":
        for a in Alumne.objects.filter(
            grup__curs__nivell=nivell, data_baixa__isnull=True
        ):
            activaAlumne(a)
            ...
            if següentCurs(a) or not ultimCursNoEmail:
                if not senseEmails:
                    mailMatricula(tipus, a.grup.curs, correus, a, connection)
    # tanca la connexió
    if connection:
        connection.close()
...
def totalsAlumnes(nany):
    ...
    quantitats = (
        Curs.objects.filter(grup__alumne__isnull=False, data_inici_curs__isnull=False)
        .distinct()
        .order_by("nom_curs_complert")
        .annotate(
            inici=Count(
                "grup__alumne",
                filter=Q(
                    (
                        Q(grup__alumne__data_baixa__isnull=True)
                        | Q(grup__alumne__data_baixa__gt=(F("data_inici_curs") + mes))
                    )
        ...
        .annotate(
            conf=Count(
                "grup__alumne",
                filter=Q(
                    Q(grup__alumne__matricula__any=nany)
...
def ResumLlistat(nany):
    ...
    cursos = (
        Curs.objects.filter(confirmacio_oberta=True)
        .distinct()
        .order_by("nom_curs_complert")
    )
    for c in cursos:
        worksheet = workbook.add_worksheet(("{0}-Confirmades".format(str(c)))[:31])
        cap = ["RALC", "Cognoms", "Nom", "Grup actual", "Curs Mat.", "Resposta"]
        worksheet.set_column(0, 0, 25)
        worksheet.set_column(1, 3, 15)
        worksheet.write_row(0, 0, cap)
        alumConf = Alumne.objects.filter(
            grup__curs=c,
            data_baixa__isnull=True,
            matricula__any=nany,
            matricula__confirma_matricula="C",
        ...
        for a in alumConf:
            ...
            worksheet.write_string(
                fila,
                4,
                str(a.matricula.get(any=nany).curs) if ConfContestada(a, nany) else "",
            )
            worksheet.write_string(
                fila,
                5,
                (
                    a.matricula.get(any=nany).get_confirma_matricula_display()
                    if ConfContestada(a, nany)
                    else "Sense resposta"
                ),
            )
            fila = fila + 1

        worksheet = workbook.add_worksheet(("{0}-No confirmades".format(str(c)))[:31])
        cap = ["RALC", "Cognoms", "Nom", "Grup actual", "Curs Mat.", "Resposta"]
        worksheet.set_column(0, 0, 25)
        worksheet.set_column(1, 3, 15)
        worksheet.write_row(0, 0, cap)
        alumNoConf = Alumne.objects.filter(
            grup__curs=c, data_baixa__isnull=True
        ).order_by("grup__nom_grup", "cognoms", "nom")
        fila = 1
        for a in alumNoConf:
            if (
                not ConfContestada(a, nany)
                or a.matricula.get(any=nany).confirma_matricula != "C"
            ):
                worksheet.write_string(fila, 0, a.ralc)
                worksheet.write_string(fila, 1, a.cognoms)
                worksheet.write_string(fila, 2, a.nom)
                ...
                worksheet.write_string(
                    ...
                    4,
                    (
                        str(a.matricula.get(any=nany).curs)
                        if ConfContestada(a, nany)
                        else ""
                    ),
                )
