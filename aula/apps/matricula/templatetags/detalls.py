...
def getTorn(matricula):
    ...
    if matricula and matricula.alumne:
        p = Preinscripcio.objects.filter(
            ralc=matricula.alumne.ralc,
            any=matricula.any,
            codiestudis=matricula.curs.nivell.nom_nivell,
            curs=matricula.curs.nom_curs,
        )
        if p:
            return p[0].torn
    return ""
...
def nomes1Fracc(pagtax):
    ...
    if pagtax and pagtax.count() == 2:
        return (pagtax[0].pagament_realitzat and not pagtax[1].pagament_realitzat) or (
            not pagtax[0].pagament_realitzat and pagtax[1].pagament_realitzat
        )
    else:
        return False
...
def ctrlTax(matricula):
    ...
    if taxes:
        pagtax = QuotaPagament.objects.filter(
