...
from aula.utils.widgets import (
...
class DadesForm1(forms.ModelForm):
    ...
    def clean(self):
        ...
        if res < -1:
            ...
            cleaned_data["alumne_correu"] = email
        return cleaned_data
...
class DadesForm2(forms.ModelForm):
    ...
    etiqueta1 = forms.CharField(
        widget=empty,
        disabled=True,
        required=False,
        label=mark_safe("Primer responsable:"),
    )
    etiqueta2 = forms.CharField(
        widget=empty,
        disabled=True,
        required=False,
        label=mark_safe("<br>Segon responsable:"),
    )
    ...
    def clean(self):
        ...
        if (
            cleaned_data.get("rp1_dni")
            or cleaned_data.get("rp1_nom")
            or cleaned_data.get("rp1_cognoms")
            or cleaned_data.get("rp1_telefon")
            or cleaned_data.get("rp1_correu")
        ):
        ...
        if (
            cleaned_data.get("rp2_dni")
            or cleaned_data.get("rp2_nom")
            or cleaned_data.get("rp2_cognoms")
            or cleaned_data.get("rp2_telefon")
            or cleaned_data.get("rp2_correu")
        ):
            ...
            if not cleaned_data.get("rp2_telefon") and not cleaned_data.get(
                "rp1_telefon"
            ):
                self.add_error("rp2_telefon", "Telèfon obligatori")
            if not cleaned_data.get("rp2_correu") and not cleaned_data.get(
                "rp1_correu"
            ):
...
class DadesForm3(forms.ModelForm):
    quotaMat = forms.CharField(
        label="Quota Matrícula:",
        widget=forms.TextInput(attrs={"readonly": True}),
        required=False,
    )
    importTaxes = forms.CharField(
        label="Import de les taxes:",
        widget=forms.TextInput(attrs={"readonly": True}),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if mat.curs.nivell.nom_nivell == "ESO":
            self.fields["fitxers"].help_text = mark_safe(
                '<b style="color:darkgreen">Targeta sanitària, carnet de vacunacions, \
                            llibre de família, documents identificació ...</b>'
            )
        else:
            self.fields["fitxers"].help_text = mark_safe(
                '<b style="color:darkgreen">Targeta sanitària, documents identificació, \
...
class ConfirmaMat(forms.ModelForm):
    ...
    etiqueta1 = forms.CharField(
        ...
        disabled=True,
        required=False,
        label=mark_safe("<br>Primer responsable:"),
    )
    etiqueta2 = forms.CharField(
        widget=empty,
        disabled=True,
    ...
    opcions = forms.ChoiceField(
        label="Continuarà el proper curs a ESO o Batxillerat ?",
        required=True,
    ...
    def clean(self):
        ...
        if opcions == "C" and not condicions:
            self.add_error(
                "acceptar_condicions",
                "És obligatori acceptar les condicions de matrícula",
            )
        ...
        if opcions == "N" and condicions is None:
            cleaned_data["acceptar_condicions"] = False
        return cleaned_data


class escollirMat(forms.Form):
    escollida = forms.ChoiceField(
        label="Selecciona la matrícula que vols fer servir",
        required=True,
        widget=forms.RadioSelect(),
    )
    ...
    def __init__(self, user, alumne, nany, *args, **kwargs):
        ...
        self.fields["escollida"].choices = [
            (
                "M",
                "Matrícula actual a {0}{1}".format(
                    mt[0].curs.nivell.nom_nivell, mt[0].curs.nom_curs
                ),
            ),
            (
                "P",
                "Fer nova matrícula segons preinscripció a {0}{1} ({2})".format(
...
def year_choices():
    ...
    if ultim:
        ...
        ultim = current_year()
    primerCurs = (
        Curs.objects.filter(data_inici_curs__isnull=False)
        .order_by("data_inici_curs")
        .first()
    )
    ultimCurs = (
        Curs.objects.filter(data_fi_curs__isnull=False).order_by("data_fi_curs").last()
    )
...
class ActivaMatsForm(forms.Form):
    ...
    datalimit = forms.DateField(
        label="Data límit", required=True, widget=DateTextImput()
    )
    tipus = forms.ChoiceField(
        label="Tipus de matrícules",
        required=False,
        choices=[
            ("P", "Amb Preinscripció"),
            ("C", "Confirmacions"),
    ...
    ultimCursNoEmail = forms.BooleanField(
        label="No envia emails a alumnes d'últim curs",
        required=False,
        help_text="Sense emails a alumnes d'últim curs, segurament ja tenen el títol.",
    )
    senseEmails = forms.BooleanField(
        label="No envia emails",
        required=False,
        help_text="Si ja s'ha donat la informació directament als alumnes. \
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["nivell"].queryset = (
            Nivell.objects.filter(
                curs__grup__alumne__isnull=False,
    ...
    def clean(self):
        ...
        if (
            tipus == "P"
            and nivell.matricula_oberta
            and nivell.limit_matricula >= django.utils.timezone.now().date()
            and Preinscripcio.objects.filter(
                estat="Enviada", codiestudis=nivell.nom_nivell, any=nany
            )
            and Preinscripcio.objects.filter(
                estat="Assignada",
            ...
            self.add_error(
                "nivell",
                "Hi ha preinscripcions vàlides d'una activació anterior. \
                           Per a poder continuar primer ha de finalitzar la matrícula activa actual a "
                + nivell.nom_nivell,
            )
        return cleaned_data
