...
from aula.apps.matricula.forms import (
    DadesForm1,
    DadesForm2,
    DadesForm2b,
    DadesForm3,
...
from aula.apps.matricula.viewshelper import (
    situacioMat,
    mailMatricula,
    següentCurs,
    quotaSegüentCurs,
...
class ConfirmaDetail(LoginRequiredMixin, UpdateView):
    ...
    def get_form(self, form_class=None):
        ...
        for f in self.fields:
            ...
            if f in modificats:
                form.fields[f].widget.attrs.update({"class": "alert alert-info"})
            else:
                form.fields[f].widget.attrs.update({"class": "alert alert-success"})
    ...
    def form_valid(self, form):
        ...
        mailMatricula(
            self.object.estat,
            self.object.curs,
            self.object.alumne.get_correus_relacio_familia(),
            self.object.alumne,
        )
...
def Confirma(request, nany):
    ...
    try:
        if user.alumne:
            ...
            if info == "D":
                return HttpResponseRedirect(
                    reverse_lazy("matricula:relacio_families__matricula__escollir")
                )
            if info == "M":
                return HttpResponseRedirect(
                ...
                if request.method == "POST":
                    ...
                    if form.is_valid():
                        ...
                        item.quota = quotaSegüentCurs(
                            settings.CUSTOM_TIPUS_QUOTA_MATRICULA, nany, user.alumne
                        )
                        item.save()
...
def changeEstat(request, pk, tipus):
    ...
    if not mat.acceptar_condicions and mat.confirma_matricula != "N":
        ...
        errors = []
        errors.append(
            "No es pot finalitzar matrícula que no té acceptades les condicions"
        )
        enviaMissatge(
            "No es pot finalitzar matrícula:{0}-{1}. No té acceptades les condicions.".format(
                mat.idAlumne, mat.any
            )
        )
        return render(
            request,
    ...
    if mat.confirma_matricula != "N":
        mailMatricula(
            mat.estat,
            mat.alumne.grup.curs,
            mat.alumne.get_correus_relacio_familia(),
            mat.alumne,
        )
    return HttpResponseRedirect(
        reverse_lazy(
...
class MatriculesList(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Matricula.objects.filter(tipusfiltre).order_by(
            "curs__nom_curs_complert", "cognoms", "nom"
        )
...
class DadesView(LoginRequiredMixin, SessionWizardView):
    ...
    def process_step_files(self, form):
        ...
        for key in files.keys():
            for value in files.getlist(key):
                # Elimina accents
                newname = (
                    unicodedata.normalize("NFKD", value.name)
                    .encode("ascii", "ignore")
                    .decode("UTF-8")
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        pk = self.kwargs.get("pk", None)
        mat = Matricula.objects.get(pk=pk)
        pagament = QuotaPagament.objects.filter(
            alumne=mat.alumne, quota=mat.quota
        ).order_by("dataLimit")
        ...
        if step == "3" or (step == "2" and self.steps.count == 3):
            ...
            if step == "3":
                ...
                if step2data:
                    ...
                    tcomplet = QuotaCentre.objects.filter(
                        any=mat.any, tipus__nom="taxcurs"
                    )
            ...
            docs = list(
                Document.objects.filter(matricula=mat)
                .order_by("pk")
                .values_list("fitxer", flat=True)
            )
            files = []
            for d in docs:
                files.append(d)
    ...
    def done(self, form_list, **kwargs):
        ...
        grup, _ = creaGrup(
            mat.curs.nivell.nom_nivell, mat.curs.nom_curs, "-", None, None
        )
        ...
        infos.append(
            "Dades completades, una vegada siguin revisades per secretaria rebrà un missatge. Es poden afegir més documents tornant a l'opció de Matrícula. "
            "Gestioni els pagaments des de l'apartat " + url
        )
        ...
        if user.alumne:
            ...
            if info == "M":
                ...
                p = Preinscripcio.objects.filter(
                    ralc=user.alumne.ralc, any=nany, estat="Enviada"
                )
                ...
                if mat.estat == "A":
                    nomAlumne = (
                        (mat.nom + " " + mat.cognoms)
                        if mat.nom and mat.cognoms
                        else mat.idAlumne
                    )
                    titol = (
                        "Dades de matrícula de "
                        + nomAlumne
                        + " a "
                        + mat.curs.nivell.nom_nivell
                        + (
                            ("(" + mat.preinscripcio.torn + ")")
                    ...
                    if mat.curs.nivell.nom_nivell in settings.CUSTOM_NIVELLS["CICLES"]:
                        form_list = [DadesForm1, DadesForm2, DadesForm2b, DadesForm3]
                        initial = {
                            "0": dict([(f, getattr(mat, f)) for f in fields0]),
                    ...
                    return DadesView.as_view(initial_dict=initial, form_list=form_list)(
                        request, pk=mat.pk, titol=titol
                    )
            else:
                infos.append(info)
        else:
        ...
        if info == "D":
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    if escollida == "M":
                        Preinscripcio.objects.filter(
                            ralc=user.alumne.ralc, any=nany, estat="Enviada"
                        ).update(estat="Caducada")
                    else:
                        p = Preinscripcio.objects.get(
                            ralc=user.alumne.ralc, any=nany, estat="Enviada"
                        )
...
def ActivaMatricula(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            nany = django.utils.timezone.now().year
            if tipus == "P":
                llista = Preinscripcio.objects.filter(
                ...
                if preexclusiva:
                    Curs.objects.filter(nivell=nivell).update(
                        limit_confirmacio=None, confirmacio_oberta=False
                    )
                nivell.save()
            if tipus == "C":
                Curs.objects.filter(nivell=nivell).update(
                    limit_confirmacio=datalimit, confirmacio_oberta=True
                )
            ...
            infomat = (
                "Matrícula activada de tipus {0} per {1} amb data límit {2}".format(
                    definicions.get(tipus, ""),
                    nivell.nom_nivell,
        ...
        if (
            not hasattr(settings, "CUSTOM_TIPUS_QUOTA_MATRICULA")
            or not bool(settings.CUSTOM_TIPUS_QUOTA_MATRICULA)
            ...
            return render(
                ...
                "resultat.html",
                {
                    "msgs": {
                        "errors": [
                            "Falta definir CUSTOM_TIPUS_QUOTA_MATRICULA o el TPV 'centre'",
                        ],
                        "warnings": [],
                        "infos": infos,
                    }
