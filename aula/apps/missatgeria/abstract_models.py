...
class AbstractMissatge(models.Model):
    ...
    text_missatge = models.TextField("Missatge", help_text="Escriu el missatge")
...
class AbstractDestinatari(models.Model):
    ...
    importancia = models.CharField(
        max_length=2, choices=IMPORTANCIA_CHOICES, default="IN"
    )
    ...
    def llegit(self):
        return self.moment_lectura is not None

    def llegitAraMateix(self):
        araMateix = False
        try:
            araMateix = (
                not self.moment_lectura
                or 0 <= (datetime.now() - self.moment_lectura).seconds <= 2
            )
        except:  # noqa: E722
            araMateix = True
        return araMateix
