...
from aula.apps.relacioFamilies.notifica import enviaEmailFamilies, FitxerSuperaMida
import collections
...
def elMeuMur(request, pg, tipus="all"):
    ...
    if tipus.upper() == "TOT":
        tipus = "all"
    q = (
        user.destinatari_set.filter(missatge__tipus_de_missatge=tipus.upper()).order_by(
            "-missatge__data"
        )
        if tipus != "all"
        else user.destinatari_set.order_by("-missatge__data")
    )
...
def enviaMsg(user, credentials, alumne, datai, horai, dataf, horaf, motiu, observ):
    ...
    msg.enllac = "/tutoria/justificaFaltes/{0}/{1}/{2}/{3}".format(
        alumne.pk, datai.year, datai.month, datai.day
    )
    msg.tipus_de_missatge = tipusMissatge(msg.text_missatge)
    msg.text_missatge = msg.text_missatge.format(
        alumne,
        datai.strftime("%d/%m"),
        horai.strftime("%H:%M"),
        dataf.strftime("%d/%m"),
...
def enviaMissatgeTutors(request):
    ...
    if request.method == "POST":
        ...
        if formAlumne.is_valid() and msgForm.is_valid() and formData.is_valid():
            ...
            if len(tutors) == 0:
                formAlumne._errors.setdefault(NON_FIELD_ERRORS, []).append(
                    """No trobat el tutor d'aquest alumne. Cal trucar al cap d'estudis."""
                )
            else:
                ...
                msg.save()
                msg.afegeix_info("Alumne: {alumne}".format(alumne=alumne))
                msg.afegeix_info("Data relativa a l'avís: {data}".format(data=data))
                msg.afegeix_info("Avís: {txt2}".format(txt2=txt2))
                for tutor in tutors:
                    msg.envia_a_usuari(tutor.getUser(), "IN")
                strTutors = ", ".join("Sr(a) {}".format(tutor) for tutor in tutors)
                ...
                messages.info(
                    request,
                    "Avís als tutors de {} ({}) enviat correctament".format(
        ...
        consergeria_darrera_data = request.session.get(
            "consergeria_darrera_data", datetime.today().strftime("%d/%m/%Y")
        )
        formAlumne = triaAlumneSelect2Form()
        formData = dataForm(
            label="Data",
            help_text="El text del missatge començarà per: Amb data ______, ",
    ...
    return render(
        ...
        {
            "formset": formset,
            "titol_formulari": "Missatge a professors tutors de l'alumne",
            "head": "Avís a tutors.",
        },
    )
...
def enviaMissatgeAdministradors(request):
        ...
        if msgForm.is_valid():
            ...
            if len(administradors) == 0:
                msgForm._errors.setdefault(NON_FIELD_ERRORS, []).append(
                    """No estan definits els administradors, sorry."""
                )
            else:
                ...
                for administrador in administradors:
                    ...
                    strAdmins += separador + "Sr(a)" + unicode(administrador)
                    separador = ", "
    ...
    return render(
        ...
        {
            "formset": formset,
            "head": """Avís a administradors. En cas d'error, sisplau, detalla clarament totes les passes per reproduir l'error.""",
        },
    )
...
def enviaMissatgeProfessorsPas(request):
    ...
    if request.method == "POST":
        ...
        if formProfessorConserge.is_valid() and msgForm.is_valid():
            ...
            msg2 = Missatge(
                remitent=user,
                text_missatge=missatge.format(destinataris_txt, msg.text_missatge),
                tipus_de_missatge=tipus_de_missatge,
            )
            ...
            messages.info(
                request,
                "Missatge a {destinataris} enviat correctament".format(
    ...
    return render(
        ...
        {
            "formset": formset,
            "titol_formulari": "Missatge a professors i/o PAS",
            "head": "Missatge a membres del professorat o consergeria",
        },
    )
...
def EmailFamilies(request):
        ...
        if msgForm.is_valid():
            ...
            try:
                ...
                missatge = EMAIL_A_FAMILIES + (
                    "\nNo enviat a {0} adreces. Els missatges pendents seran enviats més endavant.".format(
                        contErr
                    )
                    if contErr > 0
                    else ""
                ...
                missatge = missatge.format(
                    ...
                    + "\n"
                    + (
                        "\nadjunts:" + str([f.name for f in attach if f.name])
                        if attach
                        else ""
                    ),
                )
                messages.info(
                    request,
                    "Email a famílies enviat a {0} adreces".format(contOk)
                    + (
                        ", no enviat a {0} adreces. Els missatges pendents seran enviats més endavant.".format(
                            contErr
                        )
            ...
            except FitxerSuperaMida as e:
                missatge = "Email a famílies no enviat. " + str(e)
                messages.error(request, missatge)
            except Exception as e:
                missatge = (
                    "Email a famílies no enviat, torneu a intentar-ho en uns minuts. "
                    + str(e)
                )
                messages.error(request, missatge)
    ...
    return render(
        ...
        {
            "formset": formset,
            "titol_formulari": "Email per a les famílies",
            "head": "Email a totes les famílies",
        },
    )
