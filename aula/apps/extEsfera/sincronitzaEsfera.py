...
def sincronitza(f, user=None):
    ...
    col_indexs = {
        n: cell.value.strip()
        for n, cell in enumerate(rows[5])
        if cell.value and cell.value.strip() in colnames
    ...
    for row in rows[6 : max_row - 1]:  # la darrera fila també és brossa
            ...
            if index in col_indexs:
                ...
                if col_indexs[index].endswith("Data naixement"):
                    try:
                        ...
                        a.data_neixement = datetime.fromtimestamp(
                            time.mktime(dia)
                        ).date()
                ...
                if col_indexs[index].endswith("Contacte 1er tutor alumne - Valor"):
                    ...
                    if dades_tutor1["mobils"]:
                        ...
                        a.altres_telefons = posarDada(
                            dades_tutor1["fixes"], a.altres_telefons
                        )
                ...
                if col_indexs[index].endswith("Contacte 2on tutor alumne - Valor"):
                    ...
                    if dades_tutor2["mobils"]:
                        ...
                        a.altres_telefons = posarDada(
                            dades_tutor2["fixes"], a.altres_telefons
                        )
                ...
                if col_indexs[index].endswith("Contacte altres alumne - Valor"):
                    ...
                    a.altres_telefons = posarDada(
                        telefons_alumne["fixes"], a.altres_telefons
                    )
                    a.altres_telefons = posarDada(
                        telefons_alumne["mobils"], a.altres_telefons
                    )
                ...
                if col_indexs[index].endswith("Tutor 1 - tipus via"):
                    r1["adreca"] = (unicode(cell.value) if cell.value else "") + r1.get(
                        "adreca", ""
                    )
                if col_indexs[index].endswith("Tutor 1 - nom via"):
                    r1["adreca"] = r1.get("adreca", "") + (
                        (" " + unicode(cell.value)) if cell.value else ""
                    )
                ...
                if col_indexs[index].endswith("Tutor 2 - tipus via"):
                    r2["adreca"] = (unicode(cell.value) if cell.value else "") + r2.get(
                        "adreca", ""
                    )
                if col_indexs[index].endswith("Tutor 2 - nom via"):
                    r2["adreca"] = r2.get("adreca", "") + (
                        (" " + unicode(cell.value)) if cell.value else ""
                    )
        ...
        if (
            ...
            if alumneDadesAnteriors.primer_responsable == 1 and resp2:
                a.responsable_preferent = resp2
            else:
                a.responsable_preferent = resp1
            a.save()
        # DEPRECATED ^^^
