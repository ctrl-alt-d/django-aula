...
class CanviDadesUsuari(ModelForm):
    ...
    def clean(self):
        totInformat = (
            ...
            and self.cleaned_data.get("last_name")
...
class CanviDadesAddicionalsUsuari(ModelForm):
    ...
    def save(self):
        ...
        try:
            ...
            new_file_name = (
                "profe_"
                + str(self.instance.professor.username)
                + os.path.splitext(self.instance.foto.name)[1]
            )
...
class triaUsuariForm(forms.Form):
    professor = forms.ModelChoiceField(
        ...
        required=True,
        help_text="Tria en quin usuari et vols convertir.",
...
class loginUsuariForm(forms.Form):
    usuari = forms.CharField(required=True, help_text="Nom d'usuari")
    paraulaDePas = forms.CharField(
        label="Paraula de Pas",
        required=True,
        help_text="Paraula de pas",
...
class recuperacioDePasswdForm(forms.Form):
    p1 = forms.CharField(
        label="Nova Paraula de Pas",
        required=True,
        help_text="Paraula de pas, més de 6 caracters.",
        widget=forms.PasswordInput(),
    )
    p2 = forms.CharField(
        label="Repeteix Paraula de Pas",
        required=True,
        help_text="Paraula de pas, més de 6 caracters.",
        widget=forms.PasswordInput(),
    )
    data_neixement = forms.DateField(
        ...
        help_text="Data de naixement de l'alumne (Format dd/mm/yyyy)",
...
class canviDePasswdForm(forms.Form):
    p0 = forms.CharField(
        label="Paraula de pas actual",
        required=True,
        help_text="Paraula de pas actual, la que tens actualment.",
        widget=forms.PasswordInput(),
    )
    p1 = forms.CharField(
        label="Nova Paraula de Pas",
...
class sendPasswdByEmailForm(forms.Form):
    email = forms.EmailField(
        ...
        required=True,
        help_text="Entra el teu correu electrònic.",
    )
