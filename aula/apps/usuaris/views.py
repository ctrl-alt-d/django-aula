...
from aula.apps.usuaris.forms import (
    CanviDadesAddicionalsUsuari,
    CanviDadesUsuari,
    canviDePasswdForm,
    loginUsuariForm,
    recuperacioDePasswdForm,
    sendPasswdByEmailForm,
    triaProfessorSelect2Form,
    triaUsuariForm,
)
from aula.apps.usuaris.models import (
    Accio,
    AlumneUser,
    DadesAddicionalsProfessor,
    GetDadesAddicionalsProfessor,
    LoginUsuari,
    OneTimePasswd,
    Professor,
    QRPortal,
...
from aula.utils.tools import getClientAdress, unicode
...
def canviDadesUsuari(request):
    ...
    if User2Professor(user):
        ...
        dadesaddicionalsprofessor, created = (
            DadesAddicionalsProfessor.objects.get_or_create(professor=professor)
        )
    ...
    if request.method == "POST":
        formUsuari = CanviDadesUsuari(request.POST, instance=user)
        formUsuari.fields["first_name"].label = "Nom"
        formUsuari.fields["last_name"].label = "Cognoms"

        if professor:
            formDadesAddicionals = CanviDadesAddicionalsUsuari(
                request.POST, request.FILES, instance=dadesaddicionalsprofessor
            )
        ...
        if formUsuari.is_valid():
            ...
            if res < -1:
                errors.setdefault("email", []).append("""Adreça no vàlida""")
        ...
        if professor:
            dadesaddicionalsprofessor = DadesAddicionalsProfessor.objects.get(
                professor=professor
            )
            formDadesAddicionals = CanviDadesAddicionalsUsuari(
                instance=dadesaddicionalsprofessor
            )
            formDadesAddicionals.fields["foto"].label = "Foto"
    ...
    head = """Dades d'usuari"""
    infoForm = [
        ("Codi Usuari", user.username),
    ]

    if formDadesAddicionals:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.INFO,
                "Canviat el Pass de {usuari}({username}), nou passwd és [{passwd}]".format(
                    usuari=usuari, passwd=passwd, username=usuari.username
                ),
            )
...
def impersonacio(request):
    ...
    if request.method == "POST":
        ...
        formckbx = ckbxForm(
            data=request.POST,
            label="Accés de nivell 4(UAT)",
            help_text="""Marca aquesta cassella per realitzar les operacions
        ...
        if form.is_valid() and formckbx.is_valid():
            ...
            if form.cleaned_data["professor"]:
                request.session["impersonacio"] = (
                    form.cleaned_data["professor"].getUser().pk
                )
            ...
            if request.session.has_key("impersonacio"):
                ...
                if user and (
                    ...
                    or Group.objects.get_or_create(name="administradors")[0]
                    in user.groups.all()
                ):
                    messages.info(
                        request,
                        SafeText(
                            "No es pot fer impersonació com usuari administrador."
                        ),
                    )
        ...
        formckbx.fields["ckbx"].initial = (
            request.session.has_key("l4") and request.session["l4"]
        )
    formset = [form, formckbx]
    return render(request, "formset.html", {"formset": formset, "head": head})
...
def elsProfessors(request):
    ...
    capcelera.contingut = "Professor"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 60
    capcelera.contingut = "%Passa llista"
    taula.capceleres.append(capcelera)
    ...
    for professor in Professor.objects.all():
        ...
        camp.contingut = "{0:.0f}% ({1} classes impartides, {2} controls)".format(
            ...
            if user is not None:
                ...
                if (
                    ...
                    and not user.groups.filter(
                        name__in=acces_restringit_a_grups
                    ).exists()
                ):
                    LoginUsuari.objects.create(
                        usuari=user, exitos=False, ip=client_address
                    )  # TODO: truncar IP
                    form._errors.setdefault(NON_FIELD_ERRORS, []).append(
                        """El sistema està en mode restringit."""
                    )
                else:
                    if user.is_active:
                        login(request, user)
                        LoginUsuari.objects.create(
                            usuari=user, exitos=True, ip=client_address
                        )  # TODO: truncar IP
                        ...
                        form._errors.setdefault(NON_FIELD_ERRORS, []).append(
                            """Aquest compte està desactivat. Punxa a l'enllaç de recuperar contrasenya."""
                        )
            else:
                ...
                try:
                    ...
                    if user.is_active:
                        ...
                        superaFallos = (
                            logins_anteriors.count() == limitLoginFail
                            and all(not x.exitos for x in logins_anteriors)
                        )
                        if superaFallos and user.is_active:
                            ...
                            try:
                                ...
                                alumne.motiu_bloqueig = (
                                    "Error reiterat entrant la contrasenya."
                                )
                                alumne.save()
                            except AlumneUser.DoesNotExist:
                                pass
                        form._errors.setdefault(NON_FIELD_ERRORS, []).append(
                            "Usuari o paraula de pas incorrecte"
                        )
                    else:
                        form._errors.setdefault(NON_FIELD_ERRORS, []).append(
                            """Aquest compte està desactivat. Punxa a l'enllaç de recuperar contrasenya."""
...
def canviDePasswd(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if userOK is not None:
                ...
                form._errors.setdefault(NON_FIELD_ERRORS, []).append(
                    "Comprova que la paraula de pas actual proporcionada sigui la correcta."
                )
    ...
    return render(
        request,
        "form.html",
        {"form": form, "infoForm": infoForm, "head": "Canvi de Contrasenya"},
    )
...
def alumneRecoverPasswd(request, username, oneTimePasswd):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                if almn and MatriculaOberta(alumneUser.getAlumne()):
                    codiOK = OneTimePasswd.objects.filter(
                        usuari=alumneUser.getUser(), clau=oneTimePasswd
                    )
                else:
            ...
            if not alumneOK:
                errors.append(
                    "Dades incorrectes. Demaneu un altre codi de recuperació. Si el problema persisteix parleu amb el tutor."
                )
            elif codiOK and not dataOK:
                errors.append("La data proporcionada no és correcta.")
                codiOK[0].reintents += 1
                codiOK[0].save()
            elif dataOK and not codiOK:
                errors.append(
                    "L'enllaç que esteu utilitzant està caducat o no és correcte. Demaneu un altre codi de recuperació."
                )
            elif not dataOK and not codiOK:
                errors.append(
                    "Dades incorrectes. Demaneu un altre codi de recuperació."
                )
                # todoBloquejar oneTimePasswd
            elif almn and alumneUser.alumne.esBaixa():
                errors.append(
                    "Cal parlar amb el tutor per recuperar accés a aquest compte."
                )
                ...
                if alumneUser.responsable.motiu_bloqueig:
                    alumneUser.responsable.motiu_bloqueig = ""
            ...
            if not errors:
                ...
                LoginUsuari.objects.create(
                    usuari=alumneUser, exitos=False, ip=client_address
                )
    ...
    return render(
        request,
        "form.html",
        {"form": form, "infoForm": infoForm, "head": "Recuperació de Contrasenya"},
    )
...
def sendPasswdByEmail(request):
    ...
    return render(
        request, "form.html", {"form": form, "head": "Recuperació de Contrasenya"}
    )
...
def cercaProfessor(request):
    ...
    if request.method == "POST":
        formUsuari = triaProfessorSelect2Form(
            request.POST
        )  # todo: multiple=True (multiples profes de cop)
        if formUsuari.is_valid():
...
def comparteixCalendari(request, clau):
    ...
    try:
    ...
    except DadesAddicionalsProfessor.DoesNotExist:
        ...
        for instance in imparticions:
            ...
            aula = (
                instance.reserva.aula.nom_aula
                if hasattr(instance, "reserva") and instance.reserva is not None
                else ""
            )
            grup = (
                instance.horari.grup.descripcio_grup
                if hasattr(instance.horari, "grup") and instance.horari.grup is not None
                else ""
            )

            summary = "{assignatura} {aula} {grup}".format(
        ...
        sortides = list(
            Sortida.objects.filter(q_professor)
        ...
        for instance in sortides:
            ...
            summary = "{ambit}: {titol}".format(
                ambit=instance.ambit, titol=instance.titol
            )
...
def detallProfessorHorari(request, pk, detall="all"):
    ...
    dadesAddicionalsProfessor = get_object_or_404(
        DadesAddicionalsProfessor, professor=professor
    )
    ...
    if dadesAddicionalsProfessor.foto:
        Accio.objects.create(
            ...
            moment=datetime.now(),
            text="""Accés a dades sensibles del profe {0} per part de l'usuari {1}.""".format(
                professor, user
            ),
        )
