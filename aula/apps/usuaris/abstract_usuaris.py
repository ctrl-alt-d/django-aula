...
class AbstractQRPortal(models.Model):
    alumne_referenciat = models.ForeignKey(
        "alumnes.Alumne",
        db_index=True,
    ...
    usuari_referenciat = models.OneToOneField(
    ...
    localitzador = models.CharField(
        max_length=4, unique=True, default="-", db_index=True
    )
    ...
    def calcula_clau_i_localitzador(self):
        for _ in range(100):
            ...
            loca_unica = not self.__class__.objects.filter(
                localitzador=self.localitzador
            ).exists()
