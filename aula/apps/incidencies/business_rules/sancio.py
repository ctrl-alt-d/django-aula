...
def sancio_pre_delete(sender, instance, **kwargs):
    ...
    if instance.instanceDB is not None and instance.instanceDB.impres:
        errors.setdefault("impres", []).append(
...
def sancio_clean(instance):
    ...
    if instance.instanceDB is not None and instance.instanceDB.impres:
        errors.setdefault("impres", []).append(
            "Aquesta sanció ja ha estat impresa. No es pot modificar."
        )
...
def sancio_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk is not None:
        ...
        apps.get_model("presencia.NoHaDeSerALAula").objects.filter(
            sancio__id=instance.pk
        )
...
def sancio_post_save(sender, instance, created, **kwargs):
    ...
    if instance.tmp__calNotificar:
        ...
        txt_msg = missatge.format(
            instance.alumne,
            instance.data_inici.strftime("%d/%m/%Y"),
            instance.data_fi.strftime("%d/%m/%Y"),
        )
        Missatge = apps.get_model("missatgeria", "Missatge")
        tipus_de_missatge = tipusMissatge(missatge)
        msg = Missatge(
            remitent=instance.professor.getUser(),
            text_missatge=txt_msg,
    ...
    while dia_iterador <= instance.data_fi:
        for franja in totes_les_franges:
            for control in ControlAssistencia.objects.filter(
