...
def preescriu():
    ...
    Incidencia.objects.filter(
        es_vigent=True, dia_incidencia__lt=dia_prescriu_incidencia
    ).update(es_vigent=False)

    Expulsio.objects.filter(
        es_vigent=True, dia_expulsio__lt=dia_prescriu_expulsio
    ).update(es_vigent=False)
