...
def qrTokens(request, pk=None):
    ...
    if pk:
        ...
        seg_es_tutor = (
            professor.tutor_set.exists()
            or professor.i.tutorindividualitzat_set.exists()
        )
    ...
    if True:  # not excepcio:
        ...
        response["Content-Disposition"] = (
            'attachment; filename="{0}-{1}-{2}.odt"'.format(
                "QR", alumne.cognoms, alumne.nom
            )
        )
...
def configuraConnexio(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if res < -1:
                errors.setdefault("correu_relacio_familia_pare", []).append(
                    """Adreça no vàlida"""
                )
            email = form.cleaned_data["correu_relacio_familia_mare"]
            res, email = testEmail(email, False)
            if res < -1:
                errors.setdefault("correu_relacio_familia_mare", []).append(
                    """Adreça no vàlida"""
                )
...
def canviParametres(request):
    ...
    infoForm = [
        ...
        (
            ...
            + alumne.rp2_telefon
            + ", "
            + alumne.altres_telefons,
        ),
        # ( 'Nom tutors', alumne.tutors),
        ("Nom tutors", alumne.rp1_nom + ", " + alumne.rp2_nom),
        # ('Correu tutors (Saga)', alumne.correu_tutors),
        ("Correu tutors (Saga)", alumne.rp1_correu + ", " + alumne.rp2_correu),
        ("Edat alumne", edatAlumne),
    ]
...
def comunicatAbsencia(request):
    ...
    if alumne:
        ...
        messages.info(request, "No és possible fer comunicats.")
    ...
    if primerdia > (ara + timedelta(days=diesantelacio)).date():
        messages.info(
            request,
            "Només es poden fer comunicats amb antelació màxima d'una setmana. La propera classe serà el dia {0}.".format(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            enviaMsg(
                user, credentials, alumne, datai, horai, dataf, horaf, motiu, observ
            )
...
def elMeuInforme(request, pk=None):
    ...
    if detall in ["all", "assistencia"]:
        ...
        for control in tots_els_controls:
            ...
            camp.contingut = (
                "{0}".format(
                    control.relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if control.relacio_familia_notificada
                else ""
            )
            camp.negreta = False if control.relacio_familia_revisada else True
            filera.append(camp)
            ...
            camp.contingut = (
                "{0}".format(
                    control.relacio_familia_revisada.strftime("%d/%m/%Y %H:%M")
                )
                if control.relacio_familia_revisada
                else ""
            )
            camp.negreta = False if control.relacio_familia_revisada else True
            filera.append(camp)
        ...
        if not semiImpersonat:
            controlsNous.filter(relacio_familia_notificada__isnull=True).update(
        ...
        for incidencia in observacions.order_by(
            ...
            camp.contingut = (
                "{0}".format(
                    incidencia.relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if incidencia.relacio_familia_notificada
                else ""
            )
            camp.negreta = False if incidencia.relacio_familia_revisada else True
            filera.append(camp)
            ...
            camp.contingut = (
                "{0}".format(
                    incidencia.relacio_familia_revisada.strftime("%d/%m/%Y %H:%M")
                )
                if incidencia.relacio_familia_revisada
                else ""
            )
            camp.negreta = False if incidencia.relacio_familia_revisada else True
            filera.append(camp)
        ...
        report.append(taula)
        if not semiImpersonat:
            observacionsNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
        ...
        report.append(taula)
        if not semiImpersonat:
            incidenciesNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
        ...
        for expulsio in expulsions.order_by("-dia_expulsio", "-franja_expulsio"):
            ...
            camp.contingut = (
                "{0}".format(
                    expulsio.relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if expulsio.relacio_familia_notificada
                else ""
            )
            camp.negreta = False if expulsio.relacio_familia_revisada else True
            filera.append(camp)
            ...
            camp.contingut = (
                "{0}".format(
                    expulsio.relacio_familia_revisada.strftime("%d/%m/%Y %H:%M")
                )
                if expulsio.relacio_familia_revisada
                else ""
            )
            camp.negreta = False if expulsio.relacio_familia_revisada else True
            filera.append(camp)
        ...
        report.append(taula)
        if not semiImpersonat:
            expulsionsNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
    ...
    if detall in ["all", "incidencies"]:
        ...
        for sancio in sancions.order_by("-data_inici"):
            ...
            camp.contingut = (
                "{0}".format(
                    sancio.relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if sancio.relacio_familia_notificada
                else ""
            )
            camp.negreta = False if sancio.relacio_familia_revisada else True
            filera.append(camp)
            ...
            camp.enllac = None
            camp.contingut = (
                "{0}".format(sancio.relacio_familia_revisada.strftime("%d/%m/%Y %H:%M"))
                if sancio.relacio_familia_revisada
                else ""
            )
            camp.negreta = False if sancio.relacio_familia_revisada else True
            filera.append(camp)
        ...
        report.append(taula)
        if not semiImpersonat:
            sancionsNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
    ...
    if detall in ["all", "dades"]:
        ...
        camp = tools.classebuida()
        camp.enllac = None
        camp.contingut = "{0}".format(alumne.data_neixement)
        filera.append(camp)
        ...
        camp = tools.classebuida()
        camp.enllac = None
        camp.contingut = "Telèfon"
        filera.append(camp)

        camp = tools.classebuida()
        camp.enllac = None
        camp.contingut = "{0}".format(alumne.altres_telefons)
        filera.append(camp)
        ...
        camp.multipleContingut = [
            (
                "{0} ({1} , {2})".format(
                    alumne.rp1_nom, alumne.rp1_telefon, alumne.rp1_mobil
                ),
                None,
            ),
            (
                "{0} ({1} , {2})".format(
                    alumne.rp2_nom, alumne.rp2_telefon, alumne.rp2_mobil
                ),
                None,
            ),
        ]
    ...
    if infSortida:
        ...
        for act in activitats.order_by("-calendari_desde"):
            ...
            filera = []
            revisada_per_la_familia = not notifica or bool(
                notifica.relacio_familia_revisada
            )
            ...
            camp.contingut = (
                "{0}".format(
                    notifica.relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if notifica and notifica.relacio_familia_notificada
                else ""
            )
            camp.negreta = negreta
            filera.append(camp)
            ...
            camp.contingut = (
                "{0}".format(
                    notifica.relacio_familia_revisada.strftime("%d/%m/%Y %H:%M")
                )
                if notifica and notifica.relacio_familia_revisada
                else ""
            )
            camp.negreta = negreta
            filera.append(camp)
        ...
        if not semiImpersonat:
            sortidesNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
    ...
    if infQuota:
        ...
        for pagquota in pagquotes.order_by(
            ...
            camp.enllac = None
            camp.contingut = (
                pagquota.data_hora_pagament.strftime("%d/%m/%Y %H:%M")
                if pagquota.data_hora_pagament and not pagquota.pagamentFet
                else ""
            )
            camp.negreta = not bool(pagquota.pagamentFet)
            filera.append(camp)
            ...
            camp.contingut = (
                pagquota.data_hora_pagament.strftime("%d/%m/%Y %H:%M")
                if pagquota.data_hora_pagament and pagquota.pagamentFet
                else ""
            )
            camp.negreta = not bool(pagquota.pagamentFet)
            filera.append(camp)
    ...
    if detall in ["all", "qualitativa"] and qualitatives_en_curs:
        ...
        for _, respostes in groupby(respostes_sort, keyf):
            ...
            camp.negreta = (
                False if bool(respostes[0].relacio_familia_revisada) else True
            )
            filera.append(camp)
            ...
            camp.contingut = (
                "{0}".format(
                    respostes[0].relacio_familia_notificada.strftime("%d/%m/%Y %H:%M")
                )
                if respostes[0].relacio_familia_notificada
                else ""
            )
            camp.negreta = (
                False if bool(respostes[0].relacio_familia_revisada) else True
            ...
            camp.contingut = (
                "{0}".format(
                    respostes[0].relacio_familia_revisada.strftime("%d/%m/%Y %H:%M")
                )
                if respostes[0].relacio_familia_revisada
                else ""
            )
            camp.negreta = (
                False if bool(respostes[0].relacio_familia_revisada) else True
        ...
        report.append(taula)
        if not semiImpersonat:
            respostesNoves.filter(relacio_familia_notificada__isnull=True).update(
                relacio_familia_notificada=ara
            )
