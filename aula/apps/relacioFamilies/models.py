...
class Responsable(models.Model):
    ...
    correu_relacio_familia = models.EmailField(
        "Correu Notifi. Tutors", help_text="Correu de notificacions", blank=True
    )
    periodicitat_faltes = models.IntegerField(
        choices=Alumne.PERIODICITAT_FALTES_CHOICES,
        blank=False,
        default=1,
        help_text="Interval de temps mínim entre dues notificacions",
    )
    periodicitat_incidencies = models.BooleanField(
        choices=Alumne.PERIODICITAT_INCIDENCIES_CHOICES,
        blank=False,
        default=True,
    ...
    def __str__(self):
        return (
            ...
            + (
                " ("
                + " , ".join(filter(None, [self.telefon, self.correu_relacio_familia]))
                + ")"
            )
        )
    ...
    def get_alumnes_associats(self):
        return self.alumnes_associats.filter(data_baixa__isnull=True).order_by(
            "cognoms", "nom"
        )
    ...
    def get_dades(self):
        return "{0} ({1},{2}) {3}".format(
            self.get_nom(),
            self.get_telefon(),
            self.get_correu_relacio(),
            (
                "Altres: {0}".format(self.get_correu_importat())
                if bool(self.get_correu_importat())
                and self.get_correu_importat() != self.get_correu_relacio()
                else ""
            ),
        )


from aula.apps.relacioFamilies.business_rules.docattach import (  #  noqa: E402
    docattach_post_delete,
)
from aula.apps.relacioFamilies.business_rules.responsable import (  #  noqa: E402
    responsable_post_save,
)
