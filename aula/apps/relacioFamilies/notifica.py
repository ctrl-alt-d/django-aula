...
def notifica():
    ...
    for alumne_id in llista_alumnes:
        try:
            ...
            adreca_mail_informada = bool(
                alumne.correu_relacio_familia_pare or alumne.correu_relacio_familia_mare
            )
            ...
            noves_expulsions = alumne.expulsio_set.exclude(estat="ES").filter(
                relacio_familia_notificada__isnull=True
            )
            ...
            fa_dies_que_no_notifiquem = (
                alumne.relacio_familia_darrera_notificacio is None
                or alumne.relacio_familia_darrera_notificacio < fa_n_dies
            )
            hiHaNovetatsPresencia = (
                alumne.periodicitat_faltes > 0
            ...
            if hiHaNovetats and adreca_mail_informada:
                ...
                try:
                    ...
                    if settings.DEBUG:
                        print("Enviant missatge a {0}".format(alumne))
                    email = EmailMessage(
                        assumpte,
                        "\n".join(missatge),
                        fromuser,  # from
                    ...
                    if settings.DEBUG:
                        print("Error enviant missatge a {0}".format(alumne))
                    enviatOK = False
            ...
            if enviatOK:
                ...
                Accio.objects.create(
                    ...
                    text="""Notifica Relació Famílies a {0}.""".format(alumne),
                )
...
def enviaEmailFamilies(assumpte, missatge, fitxers=None):
    ...
    ara = datetime.now()
    q_no_es_baixa = Q(data_baixa__gte=ara) | Q(data_baixa__isnull=True)
    q_no_informat_adreca = (
        Q(correu_relacio_familia_pare="")
        & Q(correu_relacio_familia_mare="")
        & Q(correu_tutors="")
        & Q(rp1_correu="")
        & Q(rp2_correu="")
        & Q(correu="")
    ...
    alumnesSenseCorreu = Alumne.objects.filter(q_no_es_baixa).filter(
        q_no_informat_adreca
    )
    ...
    if alumnesSenseCorreu.exists():
        for a in alumnesSenseCorreu:
            ...
            informaNoCorreus(
                tutors, a.get_user_associat(), geturlconf("TUT", a.get_user_associat())
            )
