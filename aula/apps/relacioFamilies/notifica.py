...
from aula.apps.relacioFamilies.models import DocAttach, EmailPendent
...
def notifica_pendents():
    ...
    for ep in EmailPendent.objects.all():
        ...
        _, errors, pendents = enviaEmail(
            subject=ep.subject,
            body=ep.message,
            from_email=ep.fromemail,
            bcc=llistaemails,
        ...
        if errors > 0:
            ...
            ep.save()
            connection.close()
            print("Error enviant missatge pendent")
            print(ep)
            return
        if errors == 0:
            ep.delete()
...
def notifica():
    ...
    for alumne_id in llista_alumnes:
        try:
            ...
            for usuari, tipus in destinataris:
                ...
                relacio_familia_darrera_notificacio = ultimaNotificacio(
                    usuari.get_user_associat(), alumne
                )
                ...
                noves_sortides = getNotifElements(
                    NotificaSortida.objects.filter(alumne=alumne), usuari, alumne
                )
                if settings.CUSTOM_QUOTES_ACTIVES:
                    ...
                    nous_pagaments = QuotaPagament.objects.filter(
                        Q(notificacions_familia__moment__isnull=True)
                        | (
                            Q(notificacions_familia__moment__lt=fa7dies)
                ...
                noves_incidencies = getNotifElements(
                    alumne.incidencia_set.all(), usuari, alumne
                )
                noves_expulsions = getNotifElements(
                    alumne.expulsio_set.exclude(estat="ES"), usuari, alumne
                )
                noves_sancions = getNotifElements(
                    alumne.sancio_set.filter(impres=True), usuari, alumne
                )
                noves_faltes_assistencia = getNotifElements(
                    ControlAssistencia.objects.filter(
                        alumne=alumne, estat__pk__in=presencies_notificar
                    ),
                    usuari,
                    alumne,
                )
                noves_respostes_qualitativa = getNotifElements(
                ...
                fa_dies_que_no_notifiquem = (
                    relacio_familia_darrera_notificacio is None
                    or relacio_familia_darrera_notificacio < fa_n_dies
                )
                hiHaNovetatsPresencia = (
                    periodicitat_faltes > 0
                    and fa_dies_que_no_notifiquem
                    and noves_faltes_assistencia.exists()
                )
                ...
                hiHaNovetatsIncidencies = periodicitat_incidencies and (
                    noves_incidencies.exists()
                    or noves_expulsions.exists()
                    or noves_sancions.exists()
                )
                hiHaNovetats = (
                    hiHaNovetatsQualitativa
                    or hiHaNovetatsPresencia
                    or hiHaNovetatsSortides
                    and not settings.CUSTOM_SORTIDES_OCULTES_A_FAMILIES
                    or hiHaNovetatsPagaments
                    or hiHaNovetatsIncidencies
                )
                ...
                if hiHaNovetats and adreca_mail_informada:
                    ...
                    assumpte = "{0} - Notificacions al Djau - {1}".format(
                    ...
                    missatge = [
                        ...
                        "Benvolgut/da,",
                        "",
                        "Us comuniquem que teniu noves notificacions de l'alumne {0} a l'aplicació Djau del centre {1}".format(
                            alumne.nom + " " + alumne.cognoms, urlDjangoAula
                        ),
                        "",
                        "Recordeu que el vostre nom d'usuari és: {0}".format(
                            usuari.username
                        ),
                        "",
                        "Per qualsevol dubte que tingueu al respecte poseu-vos en contacte amb el tutor/a.",
                        "",
                        "Cordialment,",
                        "",
                        settings.NOM_CENTRE,
                        "",
                        "{0}".format(textTutorial),
                    ]

                    try:
                        fromuser = settings.DEFAULT_FROM_EMAIL
                        if settings.DEBUG:
                            print("Enviant missatge a {0}".format(usuari))
                        email = EmailMessage(
                            assumpte,
                            "\n".join(missatge),
                            fromuser,  # from
                        ...
                        if settings.DEBUG:
                            print("Error enviant missatge a {0}".format(usuari))
                        enviatOK = False
                ...
                if enviatOK:
                    ...
                    Accio.objects.create(
                        ...
                        text="""Notifica Relació Famílies a {0}.""".format(usuari),
                    )
    ...
    if num_correus_no_enviats > 0:
        raise Exception(
            "No s'han pogut enviar {} missatges a famílies.".format(
                num_correus_no_enviats
            )
        )
...
def pendentEmail(subject, body, from_email, bcc, attachments=None):
    ...
    with transaction.atomic():
        ep = EmailPendent(
            subject=subject, message=body, fromemail=from_email, toemail=str(bcc)
        )
        ep.save()
        if attachments:
            for f in attachments:
                ...
                newname = (
                    unicodedata.normalize("NFKD", f.name)
                    .encode("ascii", "ignore")
                    .decode("UTF-8")
                )
                if f.name != newname:
...
def enviaEmail(subject, body, from_email, bcc, connection=None, attachments=None):
    ...
    email = EmailMessage(
        subject, body, from_email, reply_to=[from_email], connection=connection
    )
    cont = 0
    total = len(bcc)
    correctes = 0
    ...
    while cont < total:
        ...
        try:
            ...
            if settings.DEBUG:
                print("Enviant mail a {0} adreces".format(len(destinataris)))
            if email.send() == 1:
...
def enviaEmailFamilies(assumpte, missatge, fitxers=None):
    ...
    correus_alumnes = [
        item for sublist in list(correus_alumnes) for item in sublist if item
    ]
    # elimina repetits
    ...
    subject = "{0} - {1}".format(assumpte, settings.NOM_CENTRE)
    body = "\n".join(
        [
            "{0}".format(missatge),
            "",
    ...
    if errors > 0:
        pendentEmail(
            subject, body, from_email=fromuser, bcc=pendents, attachments=fitxers
        )
