...
def reportQualitativa(qualitativa, alumnes=[], grups=[], request=None):
        ...
        for curs in nivell.curs_set.all():
            for grup in curs.grup_set.all():
                ...
                for alumne in grup.alumne_set.filter(q_filtre_alumn).distinct():
                    ...
                    report.tutors = ", ".join(
                        ["Sr(a) " + unicode(t) for t in alumne.tutorsDeLAlumne()]
                    )
                    ...
                    for assignatura in Assignatura.objects.filter(
                        ...
                        te_tipus_assignatura = hasattr(
                            resposta.assignatura, "tipus_assignatura"
                        )
                        cacelera_txt = (
                            resposta.assignatura.tipus_assignatura.capcelera
                            if te_tipus_assignatura
                            else "Matèria"
                        )
                        ...
                        for (
                            respostaQualitativa
                        ) in RespostaAvaluacioQualitativa.objects.filter(
                    ...
                    report.materia = " / ".join(capceleres_materies)
    ...
    if not excepcio:
        ...
        response = http.HttpResponse(
            """Gremlin's ate your pdf! %s""" % html.escape(excepcio)
        )
