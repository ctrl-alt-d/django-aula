...
class qualitativaItemsForm(forms.Form):
    ...
    q1 = forms.ModelChoiceField(
        queryset=RespostaAvaluacioQualitativa.objects.none(), required=False
    )
...
class alumnesGrupForm(forms.Form):
    totElGrup = forms.BooleanField(
        required=False, help_text="""Tots els alumnes del grup"""
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["totElGrup"].label = "Tots els alumnes de " + self.etiqueta
        self.fields["alumnes"].queryset = self.queryset


class triaQualitativaForm(forms.Form):
    qualitativa = forms.ModelChoiceField(
        queryset=AvaluacioQualitativa.objects.all(),
        required=True,
        help_text="Tria qualitativa",
    )
