...
from aula.apps.avaluacioQualitativa.models import (
    ItemQualitativa,
...
from aula.apps.avaluacioQualitativa.forms import (
    qualitativaItemsForm,
    alumnesGrupForm,
    triaQualitativaForm,
)
...
def lesMevesAvaluacionsQualitatives(request):
    ...
    for qualitativa in AvaluacioQualitativa.objects.all():
        ...
        capcelera.contingut = "{0}".format(unicode(qualitativa))
        ...
        capcelera.contingut = "del {0} al {1} {2}".format(
            qualitativa.data_obrir_avaluacio, qualitativa.data_tancar_avaluacio, estat
        )
        taula.capceleres.append(capcelera)
        ...
        for (
            ...
            if qualitativa in grup.avaluacioqualitativa_set.all():
                ...
                aquestaQualitativa = (
                    Q(qualitativa=qualitativa)
                    and Q(professor=professor)
                    and Q(alumne__grup=grup)
                    and Q(assignatura=assignatura)
                )
                ...
                if teFrases or teResostes:
                    ...
                    q_alumnes_a_avaluar = (
                        q_mateix_grup
                        & q_mateixa_assignatura
                        & q_mateix_professor
                        & q_liPassemLlistaDesDe
                        & q_liPassemLlistaFinsA
                    )
                    ...
                    q_alumnes_avaluats = (
                        q_alumnes_a_avaluar
                        & q_te_respostes
                        & q_mateixa_qualitativa
                        & q_mateix_professor_qualitativa
                        & q_mateixa_assiganatura_qualitativa
                    )

                    camp = tools.classebuida()
                    nTotalAlumnes = (
                        Alumne.objects.filter(q_alumnes_a_avaluar).distinct().count()
                    )
                    nAlumnesAvaluats = (
                        Alumne.objects.filter(q_alumnes_avaluats).distinct().count()
                        if nTotalAlumnes > 0
                        else 0
                    )
                    camp.contingut = "{0}".format(
                        "{0} / {1}".format(nAlumnesAvaluats, nTotalAlumnes)
                    )
                    if nTotalAlumnes > 0:
                        camp.enllac = (
                            "/avaluacioQualitativa/entraQualitativa/{0}/{1}/{2}".format(
...
def entraQualitativa(request, qualitativa_pk, assignatura_pk, grup_pk):
    ...
    hi_ha_tipus_assignatura = (
        assignatura.tipus_assignatura is not None
        and assignatura.tipus_assignatura.capcelera
    )
    assignatura_label = (
        assignatura.tipus_assignatura.capcelera
    ...
    aquestaQualitativa = (
        Q(qualitativa__pk=qualitativa_pk)
        and Q(professor=professor)
        and Q(assignatura=assignatura)
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not bool(assignatura.nom_assignatura):
                errors.add("Cal posar un nom a l'assignatura.")
                totBe = False
            else:
                assignatura = form.save()
        else:
            errors.add("Error canviant el nom de l'assignatura.")
            totBe = False

        for alumne in alumnes:
            form = qualitativaItemsForm(
                request.POST, prefix=str(alumne.pk), itemsQualitativa=itemsQualitativa
            )
            if form.is_valid():
                try:
                    # --- TODO: Begin TX ----------------------------------------------------------------
                    ...
                    for resposta in respostes:
                        try:
                            ...
                            errors.add(
                                "Hi ha hagut un error assignant qualitativa a {alumne}. Comprova-ho.".format(
                                    alumne
                                )
                            )
            ...
            if totBe:
                missatge = "Dades actualitzades correctament"
            else:
                missatge = "Hi ha hagut errors actualitzant les dades"
    ...
    for alumne in alumnes:
        ...
        qo = ""
        respostes = list(
            alumne.respostaavaluacioqualitativa_set.filter(aquestaQualitativa).filter(
                frase_oberta=""
            )
        )
        ...
        form = qualitativaItemsForm(
            ...
            initial={"alumne": alumne, "q1": q1, "q2": q2, "q3": q3, "qo": qo},
    ...
    return render(
        ...
        {
            "formset": formset,
            "head": "Respostes avaluació qualitativa grup {0}".format(grup),
            "missatge": missatge,
            "errors": errors,
        },
    )
...
def resultats(request):
    ...
    capcelera.contingut = "{0}".format("Avaluació qualitativa")
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 65
    capcelera.contingut = "Dades"
    taula.capceleres.append(capcelera)
    ...
    for qualitativa in AvaluacioQualitativa.objects.all():
        filera = []
        camp = tools.classebuida()
        camp.contingut = "{0}".format(unicode(qualitativa))
        filera.append(camp)
        ...
        estat = "(Oberta)" if obertaLaQualitativa else "(Tancada)"
        camp.contingut = "del {0} al {1} {2}".format(
            qualitativa.data_obrir_avaluacio, qualitativa.data_tancar_avaluacio, estat
        )
    ...
    if request.method == "POST":
        ...
        for grup in qualitativa.grups.all():
            ...
            if form.is_valid():
                ...
                if form.cleaned_data["totElGrup"]:
                    grups.append(grup)
            else:
                totBe = False
