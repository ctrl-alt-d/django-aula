...
class Franja2Aula(models.Model):
    franja_kronowin = models.CharField(
        "Codi hora al kronowin(0,0=primera hora)", max_length=45, unique=True
    )
    franja_aula = models.ForeignKey(
    ...
    class Meta:
        ...
        verbose_name = "Mapeig Franja Horària"
        verbose_name_plural = "Mapejos Franjes Horàries"

    def __str__(self):
        franja = unicode(self.franja_aula) if self.franja_aula else "Sense assignar"
...
class Grup2Aula(models.Model):
    ...
    Grup2Aula = models.ForeignKey(
        Grup, null=True, related_name="grup2aulakonowin_set", on_delete=models.CASCADE
    )
...
class ParametreKronowin(models.Model):
    nom_parametre = models.CharField(
        max_length=45,
        unique=True,
        help_text="Nom paràmetre (ex: passwd, assignatures amb professor, ...)",
    )
    valor_parametre = models.CharField(max_length=240, blank=True)

    class Meta:
        ordering = ["nom_parametre"]
        verbose_name = "Paràmetre Kronowin"
        verbose_name_plural = "Paràmetres Kronowin"
