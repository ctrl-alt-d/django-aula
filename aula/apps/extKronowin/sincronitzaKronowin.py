...
from aula.apps.horaris.models import DiaDeLaSetmana, Horari
...
def sincronitza(file, usuari):
    ...
    for row in reader:
        ...
        for f2g in Franja2Aula.objects.all():
            aTotesLesFrangesKronowin.add(f2g.franja_kronowin)
        # les franges pendents:
        for f2g in aFranges.difference(aTotesLesFrangesKronowin):
            errors.append(
                "Cal assignar la franja horària kronowin '%s' a una franja horària des d'admin"
                % f2g
            )
        ...
        for g2g in Grup2Aula.objects.all():
            aTotsElsGrupsKronowin.add(g2g.grup_kronowin)
        aTotsElsGrupsKronowin.add("")
        # els grups pendents:
        for g2g in aGrups.difference(aTotsElsGrupsKronowin):
            errors.append(
                "Cal assignar el grup kronowin '%s' a un grup des d'admin" % g2g
            )
        ...
        for p2g in Professor.objects.all():
            aTotsElsProfessors.add(p2g.username)
        ...
        for p2g in aProfessors.difference(aTotsElsProfessors):
            ...
            warnings.append("Nou usuari: '%s'. Passwd: '%s'" % (p2g, passwd))
    ...
    for row in reader:
        try:
            ...
            if franja_kronowin == "0,00":
                ...
                continue
            horari.hora = Franja2Aula.objects.get(
                franja_kronowin=franja_kronowin
            ).franja_aula
            ...
            curs = None
            if grup_kronowin:
                horari.grup = Grup2Aula.objects.get(
                    grup_kronowin=grup_kronowin
                ).Grup2Aula
            ...
            assignatures_amb_professor, created = (
                ParametreKronowin.objects.get_or_create(
            ...
            for prefixe_assignatura in [
                x.strip()
                for x in assignatures_amb_professor_value_list
                if bool(x.strip())
            ]:
                cal_afegir_profe = cal_afegir_profe or codi_assignatura.startswith(
                    prefixe_assignatura.replace(" ", "")
                )
                # ---busco l'assignatura:
            codi_assignatura = (
                codi_assignatura + "-" + codi_professor
                if cal_afegir_profe
                else codi_assignatura
            )
            assignatura = None
            try:
                assignatura = Assignatura.objects.get(
                    curs=curs, codi_assignatura=codi_assignatura
                )
                ...
                if grup_kronowin:
                    assignatura.curs = Grup2Aula.objects.get(
                        grup_kronowin=grup_kronowin
                    ).Grup2Aula.curs
                    # i save:
                assignatura.save()
    ...
    fussionar_assignatures, _ = ParametreKronowin.objects.get_or_create(
        nom_parametre="fusionar assignatures",
        defaults={
            "valor_parametre": "N",
        },
    )
    if fussionar_assignatures.valor_parametre == "S":
        horaris = (
            Horari.objects.filter(es_actiu=True, grup__isnull=False)
            .values_list(
                "assignatura_id", "professor_id", "dia_de_la_setmana_id", "hora_id"
            )
            .distinct()
        )

        for assignatura_id, professor_id, dia_de_la_setmana_id, hora_id in horaris:
            horaris_hora = Horari.objects.filter(
                es_actiu=True,
    ...
    infos.append("Recorda reprogramar classes segons el nou horari")
...
def creaNivellCursGrupDesDeKronowin(file, dia_inici_curs, dia_fi_curs):
    ...
    if Curs.objects.exists():
        return {
            "errors": ["Hi ha cursos creats, no es pot realitzar aquesta operació"],
            "warnings": warnings,
            "infos": infos,
        }
    ...
    for row in reader:
        ...
        c, _ = Curs.objects.get_or_create(
            nom_curs=curs,
            nom_curs_complert="{nivell}-{curs}".format(nivell=nivell, curs=curs),
            nivell=n,
            defaults={"data_inici_curs": dia_inici_curs, "data_fi_curs": dia_fi_curs},
        )
        g, _ = Grup.objects.get_or_create(
            nom_grup=lletra,
            curs=c,
            defaults={
                "descripcio_grup": grup,
            },
        )
