...
def sincronitza(file, usuari):
	...
	fussionar_assignatures, _ = ParametreKronowin.objects.get_or_create( nom_parametre = 'fusionar assignatures' , defaults= {'valor_parametre': 'N',} )
	if fussionar_assignatures.valor_parametre == 'S':
		horaris = Horari.objects.filter(es_actiu=True, grup__isnull=False).values_list('assignatura_id', 'professor_id', 'dia_de_la_setmana_id' , 'hora_id').distinct()
		for assignatura_id, professor_id, dia_de_la_setmana_id, hora_id in horaris:
			horaris_hora = Horari.objects.filter(es_actiu=True, assignatura_id=assignatura_id, professor_id = professor_id, dia_de_la_setmana_id = dia_de_la_setmana_id, hora_id = hora_id, grup__isnull=False)
