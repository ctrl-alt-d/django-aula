...
from aula.apps.extKronowin.forms import (
    sincronitzaKronowinForm,
    Kronowin2DjangoAulaFranjaForm,
    Kronowin2DjangoAulaGrupForm,
    ...
    if request.method == "POST":
        ...
        for grup in Franja2Aula.objects.all():
            form = Kronowin2DjangoAulaFranjaForm(
                request.POST, prefix=str(grup.pk), instance=grup
            )
            formset.append(form)
            if form.is_valid():
                form.save()
        ...
        for grup in Franja2Aula.objects.all():
            form = Kronowin2DjangoAulaFranjaForm(prefix=str(grup.pk), instance=grup)
            formset.append(form)
...
def assignaGrups(request):
    ...
    if request.method == "POST":
        ...
        for grup in Grup2Aula.objects.all():
            form = Kronowin2DjangoAulaGrupForm(prefix=str(grup.pk), instance=grup)
            formset.append(form)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Accio.objects.create(
                ...
                text="""Sincronitzar horaris des d'arxiu Kronowin.""",
            )

        return render(
            request,
            "resultat.html",
            {"head": "Resultat sincronització Kronowin", "msgs": resultat},
        )
    else:
...
def creaNivellCursGrupDesDeKronowin(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            tmp_file = os.path.join(settings.MEDIA_ROOT, path)
            with open(tmp_file, "r", encoding="utf-8") as f1:
                resultat = s.creaNivellCursGrupDesDeKronowin(
                    f1,
