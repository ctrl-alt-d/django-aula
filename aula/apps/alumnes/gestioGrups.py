...
def grupsPromocionar():
    ...
    idsgrupsDesdoblaments = list(
        set(
            Agrupament.objects.annotate(grupValue=F("grup_horari__id")).values_list(
                "grupValue", flat=True
            )
        )
        - set(
            Agrupament.objects.annotate(grupValue=F("grup_alumnes__id")).values_list(
                "grupValue", flat=True
            )
        )
    )
    return Grup.objects.exclude(id__in=idsgrupsDesdoblaments).order_by(
        "descripcio_grup"
    )
...
def llistaIdsAgrupament(grupid):
    ...
    llista = set()
    q_Agrup = (
        Agrupament.objects.filter(grup_horari=grupid).values("grup_alumnes").distinct()
    )
...
def grupsPotencials(horari):
    ...
    codi_ambit = (
        horari.assignatura.tipus_assignatura.ambit_on_prendre_alumnes
        if horari.assignatura.tipus_assignatura is not None
        else "G"
    )
    ...
    if codi_ambit == "I":
        ...
        grups_potencials = (
            ...
            .filter(curs__nivell=horari.grup.curs.nivell)
            .distinct()
        )
    elif codi_ambit == "C":
        grups_potencials = (
            Grup.objects.filter(alumne__isnull=False)
            .filter(curs=horari.grup.curs)
            .distinct()
        )
        ...
        if codi_ambit == "AN":
            ...
            q_grups = (
                Grup.objects.filter(curs__nivell__in=llista)
                .filter(alumne__isnull=False)
                .distinct()
            )
