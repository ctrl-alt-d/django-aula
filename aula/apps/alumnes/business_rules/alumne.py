...
def alumne_clean(instance):
    ...
    if resps.count() == 1:
        ...
        resp1, resp2 = resps
        if (
            resp1 != instance.responsable_preferent
            and resp2 != instance.responsable_preferent
        ):
...
def alumne_post_save(sender, instance, created, **kwargs):
    if instance.user_associat is None:
        ...
        user_associat, new = AU.objects.get_or_create(
            username="almn{0}".format(instance.pk)
        )
        if new:
