...
def fusiona_alumnes_by_pk(pk, credentials=None):
    ...
    alumne_grup = (
        Alumne.objects.filter(condicio1 | condicio2 | condicio3)
        .exclude(pk=pk)
        .order_by("data_alta")
    )
...
def fusiona_alumnes(a_desti, a_fusionar, credentials=None):
    ...
    controls_desti = a_desti.controlassistencia_set.values_list(
        "impartir__pk", flat=True
    )
    ...
    for a in a_fusionar:
        ...
        sortides = Sortida.objects.filter(
            q_convocats | q_que_no_vindran | q_justificacio
        ).distinct()

        for sortida in sortides:
            sortida.credentials = credentials
            colleccions = [
                sortida.alumnes_convocats,
                sortida.alumnes_justificacio,
                sortida.alumnes_que_no_vindran,
            ]
        ...
        controls = (
            ControlAssistencia.objects.filter(alumne=a)
            .exclude(impartir__pk__in=controls_desti)
        ...
        Incidencia.objects.filter(control_assistencia__in=controls_que_sobren).update(
            control_assistencia=None
        )
        Expulsio.objects.filter(control_assistencia__in=controls_que_sobren).update(
            control_assistencia=None
        )
        ...
        if usuari_associat:
            if a_desti.get_user_associat():
                Accio.objects.filter(usuari=usuari_associat).update(
                    usuari=a_desti.get_user_associat()
                )
                LoginUsuari.objects.filter(usuari=usuari_associat).update(
                    usuari=a_desti.get_user_associat()
                )
            else:
...
def controlsRang(alumne, datai, horai, dataf, horaf):
    ...
    if datai == dataf:
        return alumne.controlassistencia_set.filter(
            ...
            impartir__horari__hora__hora_fi__lte=horaf,
        )
    else:
        return alumne.controlassistencia_set.filter(
            Q(
            ...
            | Q(
                impartir__dia_impartir=dataf, impartir__horari__hora__hora_fi__lte=horaf
            )
        )
...
def properdiaclasse(alumne, desdeDiaHora):
    dia = (
        alumne.controlassistencia_set.filter(
            Q(
                impartir__dia_impartir=desdeDiaHora.date(),
                impartir__horari__hora__hora_fi__gt=desdeDiaHora.time(),
            )
            | Q(impartir__dia_impartir__gt=desdeDiaHora.date())
        )
        .order_by("impartir__dia_impartir", "impartir__horari__hora__hora_inici")
        .first()
    )
    if dia:
        return dia.impartir.dia_impartir
    else:
        return None
...
def ultimdiaclasse(alumne, finsDia):
    dia = (
        alumne.controlassistencia_set.filter(impartir__dia_impartir__lte=finsDia)
        .order_by("impartir__dia_impartir", "impartir__horari__hora__hora_inici")
        .last()
    )
    if dia:
...
def get_hores(alumne, dia, actual=True):
    ...
    if actual and dia == ara.date():
        qdata = Q(
            impartir__dia_impartir=dia, impartir__horari__hora__hora_fi__gt=ara.time()
        )
    else:
    ...
    hores = [
        (
            str(hora.id),
            hora.hora_inici.strftime("%H:%M") + "-" + hora.hora_fi.strftime("%H:%M"),
        )
        for hora in hores
    ]
