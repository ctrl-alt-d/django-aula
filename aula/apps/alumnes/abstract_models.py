...
class AbstractAlumne(models.Model):
    ...
    def edat( self, data=None):
        if data is None:
            data = date.today()
        fnac = self.data_neixement
        return  data.year - fnac.year - (( data.month,  data.day) < (fnac.month, fnac.day))

    # amorilla@xtec.cat 
    # Retorna true si avui és l'aniversari de l'alumne. 
    ...
    def aniversari( self, data=None):
        if data is None:
            data = date.today()
        fnac = self.data_neixement
        return  (( data.month,  data.day) == (fnac.month, fnac.day)) or (not calendar.isleap(data.year) and (fnac.month, fnac.day) ==(2,29) and ( data.month,  data.day)==(2,28) )
