...
def duplicats_rpt():
    ...
    taula.titol.contingut = "Duplicacions detectades "

    taula.capceleres = []

    capcelera = tools.classebuida()
    capcelera.amplade = 30
    capcelera.contingut = "Grup duplicat"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 70
    capcelera.contingut = "Opcions"
    taula.capceleres.append(capcelera)
    ...
    for a in Alumne.objects.filter(data_baixa__isnull=True):
        ...
        if alumne_grup.count() > 1:
            ...
            camp.multipleContingut = [
                ("{0} {1}".format(ag, ag.grup.nom_grup), None) for ag in alumne_grup
            ]
            filera.append(camp)
            ...
            camp.multipleContingut = [
                ("Fusionar", "/alumnes/fusiona/{0}".format(primer_alumne.pk)),
            ]
            filera.append(camp)
