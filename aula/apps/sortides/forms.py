...
from aula.apps.sortides.models import TPV, Quota, Sortida, TipusQuota
...
class PagamentEfectiuForm(forms.Form):
    alumne = forms.CharField(
        ...
        label="Alumne/a",
        required=True,
        widget=forms.Textarea(attrs={"cols": 40, "rows": 1}),
    )
    ...
    data_hora_pagament = forms.CharField(
        label="Data/Hora pagament",
        required=True,
        widget=forms.DateInput(format="%Y-%m-%d %H:%M:%S"),
    )
...
class EscollirCursForm(forms.Form):
    ...
    curs_list = forms.ModelChoiceField(
        label="Curs",
        queryset=None,
        required=True,
    )
    tipus_quota = forms.ModelChoiceField(
    ...
    automatic = forms.BooleanField(label="Assigna automàticament", required=False)
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["curs_list"].queryset = (
            Curs.objects.filter(
        ...
        if (di and di[0] not in user.groups.all()) and (
            ad and ad[0] not in user.groups.all()
        ):
            ...
            self.fields["tipus_quota"].queryset = (
                TipusQuota.objects.exclude(nom="uf")
                .exclude(nom="taxcurs")
                .filter(quota__isnull=False, nom=user.username)
                .order_by("nom")
                .distinct()
            )
        else:
...
class PagQuotesForm(forms.Form):
    ...
    nom = forms.CharField(
        widget=forms.TextInput(attrs={"readonly": True, "style": "width:100px"}),
        required=False,
    )
    grup = forms.CharField(
        max_length=10,
        widget=forms.TextInput(attrs={"readonly": True, "style": "width:80px"}),
    )
    correu = forms.CharField(
        widget=forms.TextInput(attrs={"readonly": True}),
        required=False,
    )
    ...
    quota = ModelChoiceField(
        widget=ModelSelect2Widget(
            model=Quota,
            search_fields=(
                "importQuota__icontains",
                "descripcio__icontains",
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["quota"].widget.queryset = Quota.objects.filter(
            tipus=tipus, any=nany
        ).distinct()
...
class EscollirTPV(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        # Move the default TPV query here to avoid executing it at import time
        try:
            defecte = TPV.objects.filter(nom="centre").first()
            self.fields["tpv"].initial = defecte
        except:  # noqa: E722
            # Handle the case where the table doesn't exist yet (during migrations)
            pass
