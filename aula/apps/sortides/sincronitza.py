...
def sincronitza():
    ...
    sortides_a_sincronitzar = Sortida.objects.filter(
        estat_sincronitzacio=Sortida.NO_SINCRONITZADA
    )
    for instance in list(sortides_a_sincronitzar):
        ...
        alumnes_fora_aula = (
            (alumnesQueVenen - alumnesQueNoVenen) | alumnesJustificats
        ) or []
        ...
        while (
            bool(dia_iterador)
            and dia_iterador <= instance.data_fi
            and instance.estat in ["R", "G"]
        ):
            controls = list(
                ControlAssistencia.objects.filter(
                    alumne__in=alumnes_fora_aula,
