...
def clean_sortida(instance):
    ...
    if instance.informacio_pagament is None or instance.informacio_pagament == "":
    ...
    if instance.estat in ("R",):
        ...
        if (
            instance.esta_aprovada_pel_consell_escolar not in ["A", "N"]
    ...
    if (
        ...
        and (
            instance.preu_per_alumne is None
    ...
    if (
        instance.tipus_de_pagament == "ON"
        and settings.CUSTOM_SORTIDES_OCULTES_A_FAMILIES
    ):
        errors.append("Amb activitats ocultes no es pot fer pagament online")
...
def sortida_m2m_changed(
    ...
    if action in ("post_add", "post_clear", "post_remove"):
        ...
        if bool(justificats_que_venen):
            ...
            l_str = ", ".join([unicode(a) for a in resultat[:3]])
            if len(resultat) > 3:
                l_str += " i " + unicode(len(resultat)) + " més."
            errors.append(
