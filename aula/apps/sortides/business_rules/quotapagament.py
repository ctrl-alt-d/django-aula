...
def quotapagament_pre_save(sender, instance, **kwargs):
    ...
    if instance.id and p.filter(id=instance.id):
        return
    # Si són fraccions i només hi ha una part, el crea
    if not p.filter(fracciona=False) and instance.fracciona and p.count() == 1:
        return
    # En altre cas, error
    raise ValidationError("Ja existeix el pagament.")
