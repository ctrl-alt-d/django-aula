...
class Table2_Sortides(tables.Table):
    ...
    n_acompanyants = tables.TemplateColumn(
        verbose_name="Acompanyants",
        template_code="{{record.n_acompanyants}}",
        orderable=False,
    )

    professor_que_proposa = tables.Column(
        order_by=("professor_que_proposa.last_name", "professor_que_proposa.first_name")
    )
    ...
    def __init__(self, data, origen, *args, **kwargs):
        ...
        # if origen == "Gestio":
        self.columns["n_acompanyants"].column.template_code = (
            "{{record.nom_acompanyants}}"
        )
