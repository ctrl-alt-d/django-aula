...
from aula.apps.extPreinscripcio.models import Nivell2Aula, Preinscripcio
    ...
    if index in col_indexs and value:
        ...
        if isinstance(field, dict):
            ...
            if "case" in field and field["case"]:
                value = (
                    str(value).lower() if field["case"] == "L" else str(value).upper()
                )
            if "append" in field and field["append"]:
                ...
                if field["field"] not in preinscripcio:
                    ...
                    preinscripcio[field["field"]] = (
                        str(preinscripcio[field["field"]]) + " " + str(value)
                    )
            else:
                # Assigna directament
        ...
        if cell == "Assignada":
            ...
            if pos != -1:
                ...
                if getCampPn("codi centre", num, capçaleres, row) == codi:
                    preinscripcio.update(
                        {
                            "codiestudis": getCampPn(
                                "codi ensenyament", num, capçaleres, row
                            )
                        }
                    )
                    preinscripcio.update(
                        {
                            "nomestudis": getCampPn(
                                "nom ensenyament", num, capçaleres, row
                            )
                        }
                    )
                    preinscripcio.update(
                        {
                            "codimodalitat": getCampPn(
                                "codi modalitat", num, capçaleres, row
                            )
                        }
                    )
                    preinscripcio.update(
                        {"nommodalitat": getCampPn("modalitat", num, capçaleres, row)}
                    )
                    preinscripcio.update(
                        {"curs": getCampPn("curs", num, capçaleres, row)}
                    )
                    preinscripcio.update(
                        {"regim": getCampPn("règim", num, capçaleres, row)}
                    )
                    preinscripcio.update(
                        {"torn": getCampPn("torn", num, capçaleres, row)}
                    )
...
def sincronitza(f, resetPrevious, user=None):
    ...
    for row in rows[1:]:
        ...
        if "naixement" in preinscripcio:
            preinscripcio["naixement"] = datetime.strptime(
                preinscripcio["naixement"], "%d/%m/%Y"
            )
        ...
        if (
            peticions and acceptat(row, rows[0], preinscripcio, settings.CODI_CENTRE)
        ) or (
            not peticions
            and preinscripcio.get("estat", "") == "Validada"
            and preinscripcio.get("centreassignat", "") == settings.CODI_CENTRE
        ):
            try:
                ...
                if query:
                    ...
                    if estat == "Assignada":
                        preinscripcio.update({"estat": estat})
                    else:
    ...
    if resetPrevious and peticions:
        Preinscripcio.objects.filter(
            ...
            codiestudis__in=totsCodiEstudis,
        ).delete()
    ...
    infos.append(
        "{0} alumnes llegits de {1}".format(
            info_nAlumnesLlegits, "peticions" if peticions else "dades personals"
        )
    )
    if peticions:
        infos.append("Estudis {0}".format(sorted(totsCodiEstudis)))
        totes = Preinscripcio.objects.filter(
            any=nany,
            estat__in=[
                "Validada",
                "Assignada",
        ...
        infos.append("{0} preinscripcions totals".format(totes))
    ...
    for row in rows[1:]:
        ...
        if (
            ...
            try:
                ...
                if peticions:
                    totsCodiEstudis.add(
                        convertirCodiEstudis(preinscripcio["codiestudis"], True)
                    )
    ...
    if info_nAlumnesLlegits > 0 and peticions:
        ...
        matActives = (
            Nivell.objects.filter(
                nom_nivell__in=totsCodiEstudis,
                limit_matricula__gte=now,
                matricula_oberta=True,
            )
            .values_list("nom_nivell", flat=True)
            .distinct()
        )
        preActives = Preinscripcio.objects.filter(
            any=nany, codiestudis__in=matActives, estat="Enviada", ralc__in=totsRalc
        ).values_list("codiestudis", flat=True)
        # crea llista
        ...
        if preActives:
            return (
                True,
                "Hi ha preinscripcions vàlides amb matrícula activa. \
                          S'ha de finalitzar la matrícula activa d'aquests estudis: "
                + str(sorted(preActives)),
            )
