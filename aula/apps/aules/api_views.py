...
def getStatus(request):
    ...
    reservasDAulaAvui = ReservaAula.objects.filter(
        aula=aula, dia_reserva=datetime.today(), hora_fi__gt=ara
    )
    ...
    if outputformat == "2liniesNow":
        franjaActual = (
            FranjaHoraria.objects.filter(hora_inici__lte=ara)
            .filter(hora_fi__gte=ara)
            .first()
        )
        reserves_ara = reservasDAulaAvui.filter(hora=franjaActual)
        profes_ara = ",".join([r.usuari.username for r in reserves_ara]) or "lliure"
    ...
    if outputformat == "2liniesNext":
        seguent_franja = (
            FranjaHoraria.objects.filter(hora_inici__gte=ara)
            .order_by("hora_inici")
            .first()
        )
        reserves_despres = reservasDAulaAvui.filter(hora=seguent_franja)
        profes_despres = (
            ",".join([r.usuari.username for r in reserves_despres]) or "lliure"
        )

        content = """{franja}\n{profes}\n""".format(
            franja=seguent_franja,
    ...
    if outputformat == "2linies":
        ...
        content = """{franja1} {profes1}\n{franja2} {profes2}\n""".format(
            franja1=franjaActual,
            profes1=profes_ara,
            franja2=seguent_franja,
            profes2=profes_despres,
        )
        return HttpResponse(content, content_type="text/plain; charset=utf-8")
    ...
    for reserva in reservasDAulaAvui:
        if reserva.hora_inici < ara:
            ...
            content = content + "\nnext: \n+{0} minuts".format(difInMinuts)
        motiu = (
            reserva.get_assignatures if reserva.associada_a_impartir else reserva.motiu
        )
