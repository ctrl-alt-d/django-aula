...
class AbstractAula(models.Model):
    ...
    descripcio_aula = models.CharField(
        max_length=240,
        blank=True,
        help_text="Exemple: Aforament màxim 30 persones. Exemple: 20 Ordinadors sobretaula",
    )
    ...
    def __str__(self):
        descripcio = (
            " - {0}".format(self.descripcio_aula) if bool(self.descripcio_aula) else ""
        )
        nom_i_descripcio = "{0}{1}".format(self.nom_aula, descripcio)
...
class AbstractReservaAula(models.Model):
    ...
    def __str__(self):
        return "{aula} {hora_inici}-{hora_fi} {usuari}".format(
            aula=self.aula,
    ...
    def get_assignatures(self):
        ...
        resposta = ", ".join(
            [unicode(r.horari.assignatura) for r in imparticions_associades]
        )
        return resposta or ""
    ...
    def get_grups(self):
        imparticions_associades = self.impartir_set.all()
        resposta = ", ".join([unicode(r.horari.grup) for r in imparticions_associades])
        return resposta or ""
