...
from ....apps.missatgeria.missatges_a_usuaris import (
...
def reservaaula_clean(instance):
    ...
    if instance.es_reserva_manual:
        if not instance.aula.reservable:
            errors.setdefault("hora", []).append(
                """Aula exempta de reserves. No pot ser reservada."""
            )

    # -- No es pot reservar una aula ocupada
    if instance.es_reserva_manual:
        aulaOcupada = (
            ReservaAula.objects.filter(
                hora=instance.hora, aula=instance.aula, dia_reserva=instance.dia_reserva
            )
        ...
        if aulaOcupada:
            professorsQueOcupen = [
                reserva.usuari.first_name + " " + reserva.usuari.last_name
                for reserva in ReservaAula.objects.filter(
                    hora=instance.hora,
                    aula=instance.aula,
            ...
            errors.setdefault("hora", []).append(
                """Aula ocupada en aquesta hora per """ + ",".join(professorsQueOcupen)
            )
    ...
    if instance.es_reserva_manual:
        ...
        if data_del_passat:
            errors.setdefault("dia_reserva", []).append(
                "Compte! Aquesta data de reserva és del passat!"
            )
    ...
    if instance.es_reserva_manual and not es_reservador:
        ...
        if instance.dia_reserva > darrer_dia_reserva:
            errors.setdefault("dia_reserva", []).append(
                "Només pots reservar fins al dia {0}".format(darrer_dia_reserva)
            )
    ...
    if instance.es_reserva_manual:
        ...
        if bool(disponibilitatHoraria) and instance.hora not in disponibilitatHoraria:
            errors.setdefault("hora", []).append(
                "No està previst que es pugui reservar aquesta aula en aquest horari"
            )
...
def reservaaula_pre_delete(sender, instance, **kwargs):
    ...
    if not l4 and usuari_informat and not es_meva:
        errors.setdefault("usuari", []).append(
            "Només pots anul·lar les teves reserves."
        )
        raise ValidationError(errors)
    ...
    if not l4 and te_imparticio_associada:
        errors.setdefault(NON_FIELD_ERRORS, []).append(
            "Aquesta reserva està associada a impartir classe a un grup."
        )
        raise ValidationError(errors)
