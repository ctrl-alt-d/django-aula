...
def sincronitza(f, user=None):
    ...
    for row in reader:
        ...
        for columnName, value in iter(row.items()):
            ...
            if columnName.endswith("_IDENTIFICADOR DE L'ALUMNE/A"):
                a.ralc = uvalue
                trobatRalc = True
            if columnName.endswith("_NOM"):
                a.nom = uvalue.split(",")[1].lstrip().rstrip()  # nomes fins a la coma
                a.cognoms = uvalue.split(",")[0]
                trobatNom = True
            if columnName.endswith("_GRUPSCLASSE"):
                try:
            ...
            if columnName.endswith("_CORREU ELECTRÒNIC"):
                a.correu = uvalue
            if columnName.endswith("_DATA NAIXEMENT"):
                dia = time.strptime(uvalue, "%d/%m/%Y")
                a.data_neixement = datetime.fromtimestamp(time.mktime(dia)).date()
                trobatDataNeixement = True
            if columnName.endswith("_CENTRE PROCEDÈNCIA"):
                a.centre_de_procedencia = uvalue
            if columnName.endswith("_LOCALITAT"):
                a.localitat = uvalue
            if columnName.endswith("MUNICIPI"):
                a.municipi = uvalue
            if columnName.endswith("_TELÈFON RESP. 1"):
                if "telefon" not in r1:
                    r1["telefon"] = uvalue
                else:
                    a.altres_telefons = posarDada(uvalue, a.altres_telefons)
            if columnName.endswith("_MÒBIL RESP. 1"):
                if "telefon" not in r1:
                    r1["telefon"] = uvalue
                else:
                    anterior = r1["telefon"]
            ...
            if columnName.endswith("_TELÈFON RESP. 2"):
                if "telefon" not in r2:
                    r2["telefon"] = uvalue
                else:
                    a.altres_telefons = posarDada(uvalue, a.altres_telefons)
            if columnName.endswith("_MÒBIL RESP. 2"):
                if "telefon" not in r2:
                    r2["telefon"] = uvalue
                else:
                    anterior = r2["telefon"]
                    r2["telefon"] = uvalue
                    a.altres_telefons = posarDada(anterior, a.altres_telefons)
            if columnName.endswith("_ADREÇA ELECTR. RESP. 1"):
                r1["correu"] = uvalue
            if columnName.endswith("_ADREÇA ELECTR. RESP. 2"):
                r2["correu"] = uvalue
            if columnName.endswith("_ALTRES TELÈFONS"):
                a.altres_telefons = posarDada(uvalue, a.altres_telefons)

            if columnName.endswith("_ADREÇA"):
                a.adreca = uvalue
            if columnName.endswith("_CP"):
                a.cp = uvalue
            if columnName.endswith("_DOC. IDENTITAT"):
                dni = uvalue
            if columnName.endswith("_DOC. IDENTITAT RESP. 1"):
                r1["dni"] = uvalue[-10:]
            if columnName.endswith("_RESPONSABLE 1"):
                if len(uvalue.split(",")) > 1:
                    r1["nom"] = (
                        uvalue.split(",")[1].lstrip().rstrip()
                    )  # nomes fins a la coma
                    r1["cognoms"] = uvalue.split(",")[0]
                else:
                    r1["nom"] = (
                        uvalue.split(",")[0].lstrip().rstrip()
                    )  # nomes fins a la coma
                    r1["cognoms"] = ""
            if columnName.endswith("_PARENTIU RESP. 1"):
                r1["parentiu"] = uvalue
            if columnName.endswith("_ADREÇA RESP. 1"):
                r1["adreca"] = uvalue
            if columnName.endswith("_LOCALITAT RESP. 1"):
                r1["localitat"] = uvalue
            if columnName.endswith("_MUNICIPI RESP. 1"):
                r1["municipi"] = uvalue
            if columnName.endswith("_CP RESP. 1"):
                r1["cp"] = uvalue
            if columnName.endswith("_DOC. IDENTITAT RESP. 2"):
                r2["dni"] = uvalue[-10:]
            if columnName.endswith("_RESPONSABLE 2"):
                if len(uvalue.split(",")) > 1:
                    r2["nom"] = (
                        uvalue.split(",")[1].lstrip().rstrip()
                    )  # nomes fins a la coma
                    ...
                    r2["cognoms"] = ""
            if columnName.endswith("_PARENTIU RESP. 2"):
                r2["parentiu"] = uvalue
            if columnName.endswith("_ADREÇA RESP. 2"):
                r2["adreca"] = uvalue
            if columnName.endswith("_LOCALITAT RESP. 2"):
                r2["localitat"] = uvalue
            if columnName.endswith("_MUNICIPI RESP. 2"):
                r2["municipi"] = uvalue
            if columnName.endswith("_CP RESP. 2"):
                r2["cp"] = uvalue

        if not (trobatGrupClasse and trobatNom and trobatDataNeixement and trobatRalc):
            return {"errors": ["Falten camps al fitxer"], "warnings": [], "infos": []}
        ...
        try:
            q_mateix_ralc = Q(
                ralc=a.ralc
            )  # & Q(  grup__curs__nivell = a.grup.curs.nivell )
    ...
    AlumnesDeEsfera.filter(estat_sincronitzacio__exact="PRC").update(
    ...
    CursosManuals, _ = ParametreSaga.objects.get_or_create(
        nom_parametre="CursosManuals"
    )
    ...
    ).exclude(grup__curs__nom_curs__in=CursosManuals)
    ...
    if bool(CursosManuals) and len(CursosManuals) > 0:
        ...
        Alumne.objects.filter(
            Q(estat_sincronitzacio__exact="S-I") | Q(estat_sincronitzacio__exact="S-U"),
            grup__curs__nom_curs__in=CursosManuals,
        ).update(estat_sincronitzacio="MAN", motiu_bloqueig="")
    ...
    infos.append("{0} alumnes modificats".format(info_nAlumnesModificats))
    ...
    infos.append("{0} responsables creats".format(info_nResponsablesCreats))
    if info_nRespSenseDni > 0:
        infos.append("{0} responsables sense identificació".format(info_nRespSenseDni))
    if info_nMenorsSenseResp > 0:
        infos.append(
            "{0} alumnes menors d'edat sense responsable".format(info_nMenorsSenseResp)
        )
...
def comprovar_grups(f):
    ...
    for row in reader:
        ...
        if new:
            errors.append(
                "El grup '{grup_classe}' del Saga no té correspondència al programa. Revisa les correspondències Saga-Aula".format(
