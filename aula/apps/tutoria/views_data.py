...
def justificadorMKTable(request, year, month, day):
    ...
    for delta in [0, 1, 2, 3, 4]:
        ...
        if diferents_hores:
            diferents_hores_ordenat = sorted(
                list(diferents_hores), key=lambda x: x.hora_inici
            )
    ...
    for alumne in dades.alumnes:
        ...
        for dia, hores in dades.dia_hores.itemsEnOrdre():
            ...
            controls_alumne = [
                control
                for control in controls
                if control.impartir.dia_impartir == dia and control.alumne == alumne
            ]
            for hora in hores:
                cella = tools.classebuida()
                cella.txt = ""
                controls_alumne_hora = [
                    c for c in controls_alumne if c.impartir.horari.hora == hora
                ]
