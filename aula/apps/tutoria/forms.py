...
from aula.apps.alumnes.models import AlumneGrup, Grup
from aula.utils.widgets import DateTextImput


class elsMeusAlumnesTutoratsEntreDatesForm(forms.Form):
    grup = forms.ChoiceField(help_text="Tria un grup per veure dades del grup.")
    dataDesDe = forms.DateField(
        label="Data des de",
        initial=datetime.date.today,
        required=False,
        help_text="Rang de dates: primer dia.",
    ...
    dataFinsA = forms.DateField(
        ...
        initial=datetime.date.today,
        required=True,
        help_text="Rang de dates: darrer dia.",
        widget=DateTextImput(),
    )
...
class justificaFaltesW1Form(forms.Form):
    ...
    alumne = ModelChoiceField(
        widget=ModelSelect2Widget(
            ...
            attrs={"style": "'width': '100%'", "data-placeholder": "(Justificador)"},
        ),
        queryset=AlumneGrup.objects.all(),
        required=False,
        help_text="""Alumne al que vols justificar faltes.(Justificador per tot el grup)""",
    )

    data = forms.DateField(
        label="Data faltes a justificar",
        initial=datetime.date.today,
        required=True,
        help_text="Data on hi ha les faltes a justificar.",
        widget=DateTextImput(),
    )
...
class informeSetmanalForm(forms.Form):
    grup = forms.ModelChoiceField(
        ...
        help_text="""Tria un grup per fer l'informe.""",
    )

    data = forms.DateField(
        label="Setmana informe:",
        initial=datetime.date.today,
        required=True,
        help_text="Data on hi ha les faltes a justificar.",
        widget=DateTextImput(),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.pregunta.es_pregunta_oberta:
            ...
            self.fields["pregunta_select"].choices = [
                ...
                for x in self.pregunta.possibles_respostes.split("|")
            ]  # [('',u'---Tria---')] +
        self.fields[self.q_valida].initial = (
            self.resposta_anterior.resposta.strip() if self.resposta_anterior else None
        )
        self.fields[self.q_valida].label = "{0}".format(self.pregunta.pregunta)
        self.fields[self.q_valida].help_text = self.pregunta.ajuda_pregunta
        self.fields[self.q_valida].required = True
        if (
            self.pregunta.pregunta == "Tutor/a"
            and self.q_valida == "pregunta_oberta"
            and not self.fields[self.q_valida].initial
        ):
            self.fields[self.q_valida].initial = "{0}".format(self.tutor)
        if (
            self.pregunta.pregunta == "Curs"
            and self.q_valida == "pregunta_oberta"
            and not self.fields[self.q_valida].initial
        ):
            self.fields[self.q_valida].initial = "{0}".format(self.alumne.grup)
