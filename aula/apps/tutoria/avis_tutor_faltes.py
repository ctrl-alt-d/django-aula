...
def calcular_dades_carta(alumne, data_carta=None):
    ...
    if data_carta and data_carta > datetime.today().date():
        errors.append("Revisa la data de la carta.")
    ...
    try:
        faltes_fins_a_data = (
            alumne.controlassistencia_set.filter(impartir__dia_impartir__lte=data_carta)
    ...
    nfaltes = (
        alumne.controlassistencia_set.filter(
            impartir__dia_impartir__range=(faltes_des_de_data, faltes_fins_a_data)
        )
        .filter(estat=falta)
        .count()
    )
    ...
    perNivell = (
        True if len(settings.CUSTOM_FALTES_ABSENCIA_PER_NIVELL_NUM_CARTA) > 0 else False
    )
    if perNivell:
        faltes = settings.CUSTOM_FALTES_ABSENCIA_PER_NIVELL_NUM_CARTA.get(
            alumne.getNivellCustom()
        )
        ...
        if carta_numero > maxCartes and maxCartes > 0:
            errors.append("Aquest alumne ha arribat al màxim de cartes")
            assolitMaxCartes = True
        else:
            if llindar == 0:
                errors.append(
                    "Error triant la carta a enviar a la família. Alumne {0}, sense nombre màxim de faltes per enviar carta".formnat(
                        alumne
                    )
                )
    else:
        if False:
            ...
            2,
            3,
        ] and alumne.cursa_nivell("BTX"):
            tipus_carta = "tipus3B"
        elif carta_numero in [
            1,
            2,
            3,
        ] and alumne.cursa_nivell("CICLES"):
            tipus_carta = "tipus3D"
        elif carta_numero in [
            1,
            2,
            ...
            errors.append(
                "Error triant la carta a enviar a la família. Alumne {0}, sense nivell assignat".format(
                    alumne
                )
            )
        else:
        ...
        llindar = settings.CUSTOM_FALTES_ABSENCIA_PER_TIPUS_CARTA.get(
            tipus_carta, settings.CUSTOM_FALTES_ABSENCIA_PER_CARTA
        )
...
def avisTutorCartaPerFaltes():
    ...
    for alumne in Alumne.objects.filter(data_baixa__isnull=True):
        ...
        if (
            not dades_carta["assolitmaxcartes"]
            and dades_carta["nfaltes"] >= dades_carta["llindar"]
        ):
