...
def actuacio_clean(instance):
    ...
    if instance.pk and instance.moment_actuacio < (
        dt.datetime.now() + dt.timedelta(days=-365)
    ):
        errors[NON_FIELD_ERRORS] = [
            """Aquesta actuació és massa antiga per ser editada (Té més d' un any)"""
        ]
    ...
    if user and user.pk != instance.professional.pk:
        errors[NON_FIELD_ERRORS] = [
            """Només el professional que posa la actuació la pot modificar"""
        ]
...
def actuacio_pre_delete(sender, instance, **kwargs):
    ...
    if instance.moment_actuacio < (dt.datetime.now() + dt.timedelta(days=-7)):
        errors[NON_FIELD_ERRORS] = [
            """Aquesta actuació és massa antiga per ser esborrada (Té més d' una setmana)"""
        ]

    # PRECONDICIO: Només el professor que ha posat la falta o l'equip directiu la pot treure.
    if user and user.pk != instance.professional.pk:
        errors[NON_FIELD_ERRORS] = [
            """Només el professional que posa la actuació la pot esborrar"""
        ]

    # PRECONDICIO: No es poden esborrar si tenen contingut.
    if user and instance.actuacio:
        errors[NON_FIELD_ERRORS] = [
            """Hi ha text al motiu de l'actuació. Edita l'actuació i esborrar el contingut. Aquest control es realitza per evitar esborrats involuntaris d'informació"""
        ]
