...
def reportCalendariCursEscolarTutor(professor):
    ...
    for grup in Grup.objects.filter(
        ...
        dies = (
            ControlAssistencia.objects.filter(q_alumnes)
        ...
        report.assignatures = [
            "{0}({1})".format(a["codi_assignatura"], a["curs__nom_curs"])
...
def reportFaltesIncidencies(
    ...
    for nivell in Nivell.objects.all():
        for curs in nivell.curs_set.all():
            for grup in curs.grup_set.all():
                q_alumneTriat = Q(
                    pk__in=[a.pk for a in alumnes_informe]
                ...
                for alumne in grup.alumne_set.filter(q_filtre_alumn).distinct():
                    ...
                    report.incidencies = []
                    report.expulsions = []
                    report.correus = ", ".join(alumne.get_correus_relacio_familia())
                    ...
                    if settings.CUSTOM_NO_CONTROL_ES_PRESENCIA:
                        q_presencia = Q(estat__codi_estat__in=["P", "O"]) | Q(
                            estat__codi_estat__isnull=True
                        )
                    else:
                    ...
                    for ca in alumne.controlassistencia_set.filter(
                        q_desde & q_finsa & ~q_presencia & ~q_null
                    ).order_by("impartir__dia_impartir", "impartir__horari"):
                        ...
                        itemAssitencia.assignatura = (
                            ca.impartir.horari.assignatura.getLongName()
                        )
                    ...
                    item.item = "Faltes No Justificades"
                    item.valor = n_faltes
                    report.resum.append(item)

                    item = tools.classebuida()
                    item.item = "Faltes Justificades"
                    ...
                    for incidencia in alumne.incidencia_set.filter(
                        ...
                        try:
                            NomAssignatura = (
                                incidencia.control_assistencia.impartir.horari.assignatura.getLongName()
                    ...
                    n_expulsions = 0
                    for expulsio in alumne.expulsio_set.filter(
                        q_desde & q_finsa & ~q_esborrany
                    ):
                        NomAssignatura = ""
                        try:
                            NomAssignatura = (
                                expulsio.control_assistencia.impartir.horari.assignatura.getLongName()
                                if expulsio.es_incidencia_d_aula()
                                else ""
                            )
                        except:  # noqa: E722
                            pass
    ...
    try:
        ...
        try:
            path = os.path.join(
                settings.PROJECT_DIR, "../customising/docs/faltesIncidencies.odt"
            )
        except:  # noqa: E722
            path = os.path.join(
                os.path.dirname(__file__), "templates/faltesIncidencies.odt"
            )
    ...
    if not excepcio:
        ...
        response["Content-Disposition"] = (
            "attachment; filename=assistencia_i_incidencies.odt"
        )
    else:
        response = http.HttpResponse(
            """Els Gremlins odien el teu llistat! %s""" % html.escape(excepcio)
        )
