...
def tutorPosaExpulsioPerAcumulacio(request, pk):
    ...
    if enTe3oMes:
        ...
        try:
            ...
            missatge = """Error en generar l'expulsió: {errors}""".format(
                " - ".join(itertools.chain(*e.message_dict.values()))
            )
...
def detallTutoriaAlumne(request, pk, detall="all"):
    ...
    if detall in ["all", "dades"]:
        ...
        camp.enllac = None
        camp.contingut = (
            "{0}".format(alumne.rp1_nom)
            if alumne.primer_responsable == 0
            else "{0}".format(alumne.rp2_nom)
        )
        filera.append(camp)
        ...
        camp.enllac = None
        telefons = (
            ", ".join(filter(None, [alumne.rp1_telefon, alumne.rp1_mobil]))
            if alumne.primer_responsable == 0
            else "/ ".join(filter(None, [alumne.rp2_telefon, alumne.rp2_mobil]))
        )
        camp.contingut = "{0}".format(telefons)
        filera.append(camp)
        ...
        camp.enllac = None
        camp.contingut = (
            "{0}".format(alumne.rp1_correu)
            if alumne.primer_responsable == 0
            else "{0}".format(alumne.rp2_correu)
        )
        filera.append(camp)
        ...
        camp.enllac = None
        camp.contingut = (
            "{0}".format(alumne.rp2_nom)
            if alumne.primer_responsable == 0
            else "{0}".format(alumne.rp1_nom)
        )
        filera.append(camp)
        ...
        camp.enllac = None
        telefons = (
            ", ".join(filter(None, [alumne.rp2_telefon, alumne.rp2_mobil]))
            if alumne.primer_responsable == 0
            else "/ ".join(filter(None, [alumne.rp1_telefon, alumne.rp1_mobil]))
        )
        camp.contingut = "{0}".format(telefons)
        filera.append(camp)
        ...
        camp.enllac = None
        camp.contingut = (
            "{0}".format(alumne.rp2_correu)
            if alumne.primer_responsable == 0
            else "{0}".format(alumne.rp1_correu)
        )
        filera.append(camp)
    ...
    if detall in ["all", "seguiment"]:
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 15
        capcelera.contingut = "Següiment tutorial"
        capcelera.enllac = ""
        taula.capceleres.append(capcelera)
    ...
    for grup in grups:
        ...
        for alumne in Alumne.objects.filter(consulta_alumnes, data_baixa__isnull=True):
            ...
            carta, created = CartaAbsentisme.objects.get_or_create(
                ...
                professor=professor,
                defaults={
                    "faltes_fins_a_data": datetime(1999, 1, 1),
                    "nfaltes": 0,
                    "tipus_carta": "",
                },
            )
