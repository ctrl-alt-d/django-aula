...
from aula.apps.alumnes.models import Curs, Grup, Nivell
...
def creaAgrupaments(gruph, llista, di, df, senseGrups):
    ...
    for nomg in llista:
        ...
        if galum:
            ...
            warnings.append(
                "No es crea Agrupament, grup erroni: '%s' --> '%s'" % (nomg, str(gruph))
            )
            continue
        ...
        if not c_agrup.exists():
            ...
            agrup.save()
            warnings.append("Nou agrupament: '%s'" % (agrup))
...
def creaGrup(n, c, g, di, df):
    ...
    if n is not None:
        ...
        if nnivell:
            ...
            nivell.save()
            warnings.append("Nou nivell: '%s'" % (nivell))
    curs = None
    if c is not None:
        curs, ncurs = Curs.objects.get_or_create(nivell=nivell, nom_curs=c)
        if di is not None:
            if curs.data_inici_curs is None or (
                curs.data_inici_curs < di or curs.data_inici_curs > df
            ):
                curs.data_inici_curs = di
        if df is not None:
            if curs.data_fi_curs is None or (
                curs.data_fi_curs < di or curs.data_fi_curs > df
            ):
                curs.data_fi_curs = df
        if ncurs:
            curs.nom_curs = c
            curs.nom_curs_complert = (n + "-" + c) if c != "-" else (n + "-")
            warnings.append("Nou curs: '%s'" % (curs))
        curs.save()
    ...
    if ngrup:
        ...
        warnings.append("Nou grup: '%s'" % (grup))
        grup.save()
...
def sincronitza(xml, usuari):
    ...
    _, _ = Group.objects.get_or_create(name="direcció")
    ...
    if "document" not in dades or not dades["document"]:
        errors.append("Fitxer incorrecte")
        return {"errors": errors, "warnings": warnings, "infos": infos}
    ...
    senseGrups, _ = ParametreKronowin.objects.get_or_create(
        nom_parametre="senseGrups",
        defaults={
            "valor_parametre": "False",
        },
    )
    if senseGrups.valor_parametre == "True":
        senseGrups = True
    else:
        senseGrups = False
    ...
    ficurs = None
    if (
        ("general" in dades["document"])
        and (dades["document"]["general"])
    ...
    if not (
        ("teachers" in dades["document"])
        and dades["document"]["teachers"]
        and ("teacher" in dades["document"]["teachers"])
        and dades["document"]["teachers"]["teacher"]
    ):
        errors.append("No sha definit el professorat")
        return {"errors": errors, "warnings": warnings, "infos": infos}
    ...
    for p in dades["document"]["teachers"]["teacher"]:
        ...
        if nouProfessor:
            ...
            warnings.append("Nou usuari: '%s'. Passwd: '%s'" % (codiProfessor, passwd))
    ...
    #
    if not (
        ("classes" in dades["document"])
        and dades["document"]["classes"]
        and ("class" in dades["document"]["classes"])
        and dades["document"]["classes"]["class"]
    ):
        errors.append("No shan definit grups")
        return {"errors": errors, "warnings": warnings, "infos": infos}

    for gr in dades["document"]["classes"]["class"]:
        nomgrup = gr["@id"][3:]
        gr.get("class_teacher", "")  # tutor. De moment no es fa servir
        if " " in nomgrup:
        ...
        if galum:
            ...
            if agrupament != "" and agrupament[0] == "*":
                ...
                if senseGrups:
                    ...
                    warnings.extend(warn)
                    warn = creaAgrupaments(
                        grupc, ["CL_" + n + c + "-"], inicurs, ficurs, senseGrups
                    )
                    warnings.extend(warn)
    ...
    #
    if not (
        ("rooms" in dades["document"])
        and dades["document"]["rooms"]
        and ("room" in dades["document"]["rooms"])
        and dades["document"]["rooms"]["room"]
    ):
        errors.append("No shan definit aules")
        return {"errors": errors, "warnings": warnings, "infos": infos}
    ...
    for a in dades["document"]["rooms"]["room"]:
        ...
        if nouAula:
            ...
            rAula.save()
            warnings.append("Nova aula: '%s'" % (codiAula))
            nAulesCreades += 1
    ...
    #
    if not (
        ("lessons" in dades["document"])
        and dades["document"]["lessons"]
        and ("lesson" in dades["document"]["lessons"])
        and dades["document"]["lessons"]["lesson"]
    ):
        errors.append("No shan definit classes")
        return {"errors": errors, "warnings": warnings, "infos": infos}

    Horari.objects.update(es_actiu=False)

    for item in dades["document"]["lessons"]["lesson"]:
        mat = item.get("lesson_subject", "")
        if bool(mat):
            mat = mat["@id"][3:]
        prof = item.get("lesson_teacher", "")
        if bool(prof):
            prof = prof["@id"][3:]
        # Fusiona grups si n'hi ha varis
        ngrup = item.get("lesson_classes", "")
        grup = None
        if bool(ngrup):
            ngrup = ngrup["@id"]
        ...
        if (
            "times" in item
            and item["times"]
            and "time" in item["times"]
            and item["times"]["time"]
        ):
            cl = item.get("times").get("time")
    ...
    if (
        ("holidays" in dades["document"])
        and dades["document"]["holidays"]
        and ("holiday" in dades["document"]["holidays"])
        and dades["document"]["holidays"]["holiday"]
    ):
        for item in dades["document"]["holidays"]["holiday"]:
            desc = item.get("longname", "")
            iniPer = item.get("starttime", None)
            if iniPer:
                iniPer = datetime.datetime.strptime(iniPer, "%Y%m%d").date()
            fiPer = item.get("endtime", None)
            if fiPer:
            ...
            if iniPer and fiPer:
                ...
                if diafestiu.count() == 0:
                    diafestiu = Festiu()
                    warnings.append(
                        "Nou festiu: '%s' %s - %s" % (desc, str(iniPer), str(fiPer))
                    )
                else:
                    diafestiu = diafestiu[0]
                    if fiPer != diafestiu.data_fi_festiu:
                        warnings.append(
                            "Canvi dates festiu: '%s' %s - %s"
                            % (desc, str(iniPer), str(fiPer))
                        )
    ...
    infos.append("Importació finalitzada" + ambErrors + ambAvisos)
    infos.append(" ")
    infos.append("%d línies llegides" % (nLiniesLlegides,))
    infos.append("%d horaris creats o modificats" % (nHorarisModificats))
    infos.append("%d aules creades" % (nAulesCreades))
    infos.append("%d assignatures creades" % (nAssignaturesCreades))
    infos.append("Recorda reprogramar classes segons el canvia horari")
...
def creaHorari(
    ...
    try:
        ...
        assignatures_amb_professor_value_list = (
            assignatures_amb_professor.valor_parametre.split(",")
        )
        for prefixe_assignatura in [
            x.strip() for x in assignatures_amb_professor_value_list if bool(x.strip())
        ]:
            cal_afegir_profe = cal_afegir_profe or mat.startswith(
                prefixe_assignatura.replace(" ", "")
            )
            # ---busco l'assignatura:
        ...
        if horari.grup is not None:
            try:
                materia = Assignatura.objects.get(
                    curs=horari.grup.curs, codi_assignatura=mat
                )  # ,
                ...
                materia.nom_assignatura = mat
                materia.tipus_assignatura = TipusDAssignatura.objects.filter(
                    ambit_on_prendre_alumnes=tipus
                ).first()
                materia.save()
            ...
            try:
                materia = Assignatura.objects.get(
                    curs__isnull=True, codi_assignatura=mat
                )  # ,
                # tipus_assignatura__ambit_on_prendre_alumnes=tipus)
                materia.nom_assignatura = mat
                # Ja existeix, no canvia tipus. Si fa falta es fa manualment des d'admin
                ...
                materia = (
                    Assignatura.objects.filter(curs__isnull=True, codi_assignatura=mat)
                    .order_by("id")
                    .last()
                )
                materia.nom_assignatura = mat
        ...
        if novamat:
            if horari.grup:
                ...
                curs = None
            warnings.append("Nova assignatura: '%s' '%s'" % (mat, str(curs)))
            compAssig += 1
