...
def fact_controls(n=-1, flag_impersona=False):
    ...
    tots_els_controls_assistencia = (
        ControlAssistencia.objects.exclude(alumne__data_baixa__isnull=False)
    ...
    for pk in tots_els_controls_assistencia:
        ...
        # alumne
        cognoms_nom = element.alumne.cognoms + ", " + element.alumne.nom
        if flag_impersona:
        ...
        # professor
        element_row.append(
            element.impartir.professor_passa_llista.username
            if element.impartir.professor_passa_llista
            else element.impartir.horari.professor.username
        )

        element_row.append(
            element.impartir.professor_passa_llista.email
            if element.impartir.professor_passa_llista
            else element.impartir.horari.professor.email
        )
        ...
        nAssistencies = (
            1
            if element.estat is not None and element.estat.codi_estat in ("P", "R", "O")
            else 0
        )
        ...
        nFaltesTotes = (
            1
            if element.estat is not None and element.estat.codi_estat in ("F", "J")
            else 0
        )
        element_row.append(nFaltesTotes)

        nFaltesNoJustificades = (
            1 if element.estat is not None and element.estat.codi_estat in ("F") else 0
        )
        element_row.append(nFaltesNoJustificades)

        nFaltesJustificades = (
            1 if element.estat is not None and element.estat.codi_estat in ("J") else 0
        )
        element_row.append(nFaltesJustificades)

        nRetards = (
            1 if element.estat is not None and element.estat.codi_estat in ("R") else 0
        )
...
def dades_dissociades(element):
    ...
    # ---Una hora abans
    unaHora40abans = add_secs_to_time(
        element.impartir.horari.hora.hora_inici, -100 * 60
    )
