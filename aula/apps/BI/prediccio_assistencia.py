...
def predictTreeModel(values):
    ...
    if not hasattr(settings, "PREDICTION_TREE") or settings.PREDICTION_TREE is None:
        if (
            "assistenciaaHoraAnterior" in values
            and values["assistenciaaHoraAnterior"] == "Absent"
        ):
            return "Absent", 0.8
        else:
            return "Present", 0.5
    ...
    n_predict = next(
        x.get("recordCount")
        for x in Node
        if etree.QName(x).localname == "ScoreDistribution" and x.get("value") == predict
    )
    ...
    while True:
        ...
        try:
            ...
            fill = next(
                e
                for e in Node
                if etree.QName(e).localname == "Node"
                and unicode(values[e[0].get("field")]) == e[0].get("value")
            )
