...
for imparticio in Impartir.objects.filter(
    dia_impartir__in=dates, horari__grup__curs__nom_curs_complert__in=cursos
):  # , horari__professor = dani_herrera ):
    te_alumnes = imparticio.controlassistencia_set.exists()
    impartir_anteriors = Impartir.objects.filter(
        horari__hora=imparticio.horari.hora,
