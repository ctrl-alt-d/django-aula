...
>Una alternativa a instalar de nuevo en cada curso es la [inicialización](./USER_MANUAL/manual-de-uso/inicialitza.md).
...
* **`settings.py` (Aquí se encuentra la parametrización Custom de la app y no hay que tocar este fichero. Si se necesitan modificar o añadir `settings`, hay que hacerlo en `settings_local.py`)** Todas las posibilidades se pueden encontrar directamente dentro del archivo [advanced_settings.py](../setup_djau/advanced_settings.py), pero puede leer la documentación elaborada en un formato más humano en **[configuració avançada](./USER_MANUAL/manual-de-uso/configuracio_avancada.md).**
...
![](./USER_MANUAL/.gitbook/assets/captura.PNG)
